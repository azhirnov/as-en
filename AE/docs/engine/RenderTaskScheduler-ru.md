...
`RenderTask`, `RenderCoro` - используется для асинхронной записи командного буфера. Внутри метода `Run()` создается командный буфер, заполняется и добавляется в `CommandBatch` с помощью метода `Execute(cmdbuf)`.
...
Исходник: [RenderTask.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Private/RenderTask.h), [CommandBatch.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Private/CommandBatch.h)


## Вторичный командный буфер

`DrawTask`, `DrawCoro` - используется для асинхронной записи вторичного командного буфера, аналогично RenderTask.

`DrawCommandBatch` - хранит массив вторичных командных буферов, которые затем выполняются в `IGraphicsContext::ExecuteSecondary()`.

Исходник: [DrawTask.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Private/DrawTask.h), [DrawCommandBatch.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Private/DrawCommandBatch.h)


## Планировщик графических задач (RenderTaskScheduler)

#### Начало кадра
Метод `WaitNextFrame()` ожидает пока предыдущий кадр отправится на выполнение и кадр минус N завершит выполнение на ГП, при двойной буферизации N=1, при тройной N=2. Пока идет ожидание внутри выполняются другие задачи. При ошибке или слишком долгом ожидании метод вернет `false`.
...
Дополнительно есть вариант подписаться на момент отправки батча на ГП (вызов vkQueueSubmit), для этого используется зависимость `CmdBatchOnSubmit{batch}`, пример кода: `co_await Tuple{CmdBatchOnSubmit{batch}}`.<br/>
Для работы с доступной для ЦП памятью используется зависимость `OnFrameNextCycle`, в отличие от отдельного батча `CommandBatchPtr`, эта зависимость выполняется после вызова vkInvalidateMappedMemoryRanges, что позволяет читать из замапленой памяти.

![](img/RenderTaskScheduler-2.png)

Исходник: [RenderTaskScheduler.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Private/RenderTaskScheduler.h)


## Контекст для записи команд

Запись команд в Vulkan и Metal имеют отличия - в Metal используются энкодеры, они нужны для лучшего распараллеливания команд и сортировки их встроенным рендер графом.
Переключение между graphics и compute дорогое, также и между graphics и transfer, поэтому подход из Metal заставляет писать более оптимизированный код за счет своей архитектуры.

> Например на NVIDIA переключение между graphics и compute приводит к глобальному барьеру, исключение только async compute.
...
На Vulkan часть команд может выполняться только в графических или вычислительных очередях, но большая часть команд доступна и в async transfer очереди.<br/>
На Metal нет разделения по типам очередей, но они могут эмулироваться для совместимости с Vulkan.

Поддерживает асинхронное чтение из ГП памяти:

```cpp
ctx.ReadbackImage( image, ReadbackImageDesc{} )
    .IfFullyRead(
        [](Promise<ImageMemView> readRes) -> InlineCoro<>
        {
...
Исходник: [CommandBuffer.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/Public/CommandBuffer.h)
 ...
 * Ресурсы не удаляются сразу, а с задержкой в 2 кадра (при двойной буферизации).
 * Используется общий staging buffer, выделенная память гарантированно валидна в пределах кадра, это упростило работу с памятью. Например при чтении из видеопамяти в `ITransferContext::ReadbackImage()`.
 * Ограничен максимальный размер staging buffer на кадр, так чтобы все данные успели передаться по шине PCI-E за время кадра (1 / ожидаемый fps). Таким образом ГП не простаивает в ожидании данных с ЦП и время кадра более стабильное.
