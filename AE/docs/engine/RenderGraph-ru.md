...
Начальное и конечное состояние ресурса это либо дефолтное (`EResourceState::General`), либо задается вручную на этапе планирования рендер графа, иногда это добавляет ненужные синхронизации, но потери на них минимальные, если не приводят к декомпресии рендер таргетов.
...
Исходники: [RGCommandBatch.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/RenderGraph/RGCommandBatch.h), [RenderGraph.h](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/graphics_rhi/RenderGraph/RenderGraph.h)
...
[Пример лога](https://github.com/azhirnov/as-en/blob/dev/AE-Data/tests/graphics_rhi/vulkan/NVIDIA%20GeForce%20RTX%202080/Test_AsyncCompute2.txt).
