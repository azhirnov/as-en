...
* Для нормалей лучше использовать UNorm/SNorm формат, так как Float16 формат тратит биты на экспоненту, которая нужна только для совсем маленьких значений.
* D32F формат также дает большую точность около нуля и намного меньшую около 1, поэтому применяют ReverseZ.
...
* Если нужно обрабатывать большие объемы данных, то FS лучше, так как запись идет в рендер таргет с аппаратной компрессией, что дает выигрышь в пропускной способности при записи и при последующем чтении.
* Для FS драйвер сам определяет размер воркгруппы / тайла, в общем случае это удобно, но иногда ручная настройка может дать лучшую производительность.
