...
* Нагружает память из-за случайной записи.
...
### PowerVR Hierarchical Scheduling Technology
...
VisBuf дает наилучший результат, так как вызывается меньше FS, в 4К это позволяет рисовать без дополнительного отсечения.
...
VisBuf дает наилучший результат, но требует предварительного отсечения. 
...
|**early ZS, front to back**| 1.0    | 1.0    | 1.0    |
| DPP, back to front        | 
| DPP, front to back        | 1.4    | 1.14   | 1.34   |
...
TBR архитектура плохо справляется с неотсортированной геометрией (EarlyZS, back to front) поэтому HiZ и VisBuf показывают хорошие результаты.

TBDR архитектура сама по себе не предполагает сортировки геометрии, но производители добавили разные варианты оптимизаций для экономии энергопоторебления.
