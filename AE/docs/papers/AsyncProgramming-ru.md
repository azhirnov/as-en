...
Но можно написать все проверки в коде, так функция [CreateAsync line:2172](https://github.com/azhirnov/as-en/blob/dev/AE/engine/src/threading/TaskSystem/AsyncTask.h) на этапе компиляции проверяет аргументы и тип корутины на перечисленные выше ошибки.
...
В однопоточном OpenGL наоборот - вызовы glBufferData часто приводят к выделению новой памяти, то есть внутри создается новый имутабельный объект, для использования буфера из нескольких потоков потребовались бы постоянные синхронизации, которые в худшем случае приводят к вырождению многопоточного кода в однопоточный.
...
Для минимизации блокировок некоторые классы требуют внешней синхронизации вместо использования своего мьютекса внутри методов класса, это позволяет пользователю решать какой алгоритм синхронизации использовать: мьютексы, зависимости между тасками и тд.
