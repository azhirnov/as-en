...
790M * 1024 = 809G FMA ops per second = 1620 GFLOPS
```
...
* Buffer/Image storage 16bpp 16.7MPix 2x267MB [[7](../GPU_Benchmarks.md#7-BufferImage-storage-access)]
	...
	| 0   | **15.2** | 35 | Buffer load/store, 16 byte, wg:64                   | 16 byte cache line (?) |
	...
	|     |      |    | Image fetch/sample in CS with double buffering          |  |
	| 49  | 22.6 | 24 | Image fetch/sample in FS with double buffering          | a bit faster than input attachment because of texture cache |
	| 53  | 23.3 | 23 | Buffer load/store, 16 byte, wg:16x16                    |
	| 53  | 23.3 | 23 | Buffer load/store in FS                                 |
	| 55  | 23.5 | 23 | Image read/write input attachment RGBA32F               | RT compression is not supported, same performance as buffer in FS - cache misses because of thread order(?), immediate mode rendering should be used |
...
* RGBA16_UNorm, RGBA16F - RT compression is supported. [[3](../GPU_Benchmarks.md#3-Render-target-compression)]
	...
	| diff | exec time (ms) | approx traffic (GB/s) | name |
	|---|---|---|---|
	| 1   | 150 | | image storage |
	| 2   |  75 | | 1x1 noise     |
	|     |     | | 16x16 noise   | **same as block size** |
	| 3.3 |  45 | | solid color   |
