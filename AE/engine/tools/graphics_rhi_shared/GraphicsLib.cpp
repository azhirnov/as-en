...
namespace
{
	...
	public:
		...
		void  UploadImage (ImageID id, const UploadImageDesc &desc, Bytes maxSize, OUT ImageMemView &mem)		__Th_OV	{ _ctx.UploadImage( id, desc, maxSize, OUT mem ); }
		void  UploadImage (VideoImageID id, const UploadImageDesc &desc, Bytes maxSize, OUT ImageMemView &mem)	__Th_OV	{ _ctx.UploadImage( id, desc, maxSize, OUT mem ); }
	...
	public:
		...
		bool						CreateDescriptorSets (OUT DescSetBinding &binding, OUT Strong<DescriptorSetID> *dst, usize count, GraphicsPipelineID   ppln, DescriptorSetName::Ref dsName, DescriptorAllocatorPtr allocator, StringView dbgName, const DescSetParams* p)	__NE_OV { return _rm->CreateDescriptorSets( OUT binding, OUT dst, count, ppln, dsName, RVRef(allocator), dbgName, p ); }
		bool						CreateDescriptorSets (OUT DescSetBinding &binding, OUT Strong<DescriptorSetID> *dst, usize count, MeshPipelineID       ppln, DescriptorSetName::Ref dsName, DescriptorAllocatorPtr allocator, StringView dbgName, const DescSetParams* p)	__NE_OV { return _rm->CreateDescriptorSets( OUT binding, OUT dst, count, ppln, dsName, RVRef(allocator), dbgName, p ); }
		...
		bool						CreateDescriptorSets (OUT DescSetBinding &binding, OUT Strong<DescriptorSetID> *dst, usize count, TilePipelineID       ppln, DescriptorSetName::Ref dsName, DescriptorAllocatorPtr allocator, StringView dbgName, const DescSetParams* p)	__NE_OV { return _rm->CreateDescriptorSets( OUT binding, OUT dst, count, ppln, dsName, RVRef(allocator), dbgName, p ); }
		bool						CreateDescriptorSets (OUT Strong<DescriptorSetID> *dst, usize count, PipelinePackID packId, DSLayoutName::Ref dslName, DescriptorAllocatorPtr allocator, StringView dbgName, const DescSetParams* p)										__NE_OV { return _rm->CreateDescriptorSets( OUT dst, count, packId, dslName, RVRef(allocator), dbgName, p ); }
		bool						CreateDescriptorSets (OUT Strong<DescriptorSetID> *dst, usize count, DescriptorSetLayoutID layoutId, DescriptorAllocatorPtr allocator, StringView dbgName, const DescSetParams* p)															__NE_OV { return _rm->CreateDescriptorSets( OUT dst, count, layoutId, RVRef(allocator), dbgName, p ); }
		...
		AsyncRTechPipelines			LoadRenderTechAsync (PipelinePackID, RenderTechName::Ref, const RenderTechDesc &, PipelineCacheID)					__NE_OV	{ CHECK_MSG( false, "not supported" );  return Default; }
		RenderTechPipelinesPtr		LoadRenderTech (PipelinePackID packId, RenderTechName::Ref name, const RenderTechDesc &desc, PipelineCacheID cache)	__NE_OV	{ return _rm->LoadRenderTech( packId, name, desc, cache ); }
	...
	public:
		...
		bool  BindRayTracingPartitionedScene (UniformName::Ref name, DeviceAddress scene, uint elementIndex)							__NE_OV	{ return _du.BindRayTracingPartitionedScene( name, scene, elementIndex ); }
		bool  BindRayTracingPartitionedScene (UniformName::Ref name, BufferID buffer, Bytes offset, uint elementIndex)					__NE_OV	{ return _du.BindRayTracingPartitionedScene( name, buffer, offset, elementIndex ); }
		uint  RayTracingPartitionedSceneCount (UniformName::Ref name)																	C_NE_OV	{ return _du.RayTracingPartitionedSceneCount( name ); }
	};
