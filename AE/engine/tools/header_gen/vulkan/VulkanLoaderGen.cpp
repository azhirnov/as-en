...
=================================================
*/
	bool  VulkanLoaderGen::GenVulkanLoaders (const Path &outputFolder, Version2 minVer) const
	{
		struct Group
		{
		...
		for (auto& fn : _funcs)
		{
			...
			// add function pointer decl
			{
				...
				src << "\tstatic PFN_" << fn.data.name << "  _var_" << fn.data.name << ";\n";
			}
			...
			// add function pointer
			{
				...
				src << "\tPFN_" << fn.data.name << "  " << (is_dev ? "" : "VulkanInstanceFn::") << "_var_" << fn.data.name << " = null;\n";
			}
			...
			// add function loader
			{
				...
				src << "\tLoad( OUT " << (is_dev ? "table." : "VulkanInstanceFn::") << "_var_" << fn.data.name << ", \"" << fn.data.name << "\","
					<< " Dummy_" << fn.data.name << " );\n";
			}
...
=================================================
*/
	VulkanLoaderGen::FeatureSet  VulkanLoaderGen::_GetFeatures (Version2 minVer) const
	{
		...
		// shaders //
			...
			{ "shaderReplicatedComposites",		VK_EXT_SHADER_REPLICATED_COMPOSITES_EXTENSION_NAME,			NoVer,	{1,0},	{VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME} },
			{ "shaderBFloat16",					VK_KHR_SHADER_BFLOAT16_EXTENSION_NAME,						NoVer,	{1,0},	{VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME} },
			{ "shaderFloat8",					VK_EXT_SHADER_FLOAT8_EXTENSION_NAME,						NoVer,	{1,0},	{} },
		...
		// external memory //
			...
			{ "externalMemoryWin32",			VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME,				NoVer,	{1,0},	{VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME} },
			{ "keyedMutexWin32",				VK_KHR_WIN32_KEYED_MUTEX_EXTENSION_NAME,					NoVer,	{1,0},	{} },
...
=================================================
*/
	Nd__In bool  IsPlatformSpecificExtension (StringView extName)
	{
		return	StartsWith( extName, "VK_ANDROID_" )	or
				HasSubString( extName, "_WIN32_" );
	}
...
=================================================
*/
	String  VulkanLoaderGen::_GetFeaturesBoolStruct (const FeatureSet &feats) const
	{
		String	str;
...
=================================================
*/
	String  VulkanLoaderGen::_GetFeaturesPropsStruct (const FeatureSet &feats) const
	{
		String	str;
...
=================================================
*/
	String  VulkanLoaderGen::_GetExtensionsListFunc (const FeatureSet &feats) const
	{
...
=================================================
*/
	String  VulkanLoaderGen::_GetFeaturesAndPropertiesFunc (const FeatureSet &feats) const
	{
		String	str;
		...
		str	<< "\t\t\tif ( _extensions.externalMemoryCaps )\n"
			...
			<< "\t\t\t\tnext_props  = &_properties.deviceIdProperties.pNext;\n"
			<< "\t\t\t\t_properties.deviceIdProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ID_PROPERTIES;\n"
			<< "\t\t\t}\n";
		...
		for (auto& feat : feats.device)
		{
			...
			if (auto info = _extInfo.find( feat.extension ); info != _extInfo.end())
			{
				...
				if ( info->second.feats )
				{
					...
					const StructInfo&	feat_st = info->second.feats.value()->data;
...
=================================================
*/
	String  VulkanLoaderGen::_GetLogFeaturesFunc (const FeatureSet &feats) const
	{
		String	str;
...
=================================================
*/
	String  VulkanLoaderGen::_GetCheckFeaturesFunc (const FeatureSet &feats) const
	{
		constexpr Version2	NoVer = Version2::Max();
...
=================================================
*/
	bool  VulkanLoaderGen::GenVulkanFeatures (const Path &outputFolder, Version2 minVer) const
	{
		const auto	feats = _GetFeatures( minVer );
