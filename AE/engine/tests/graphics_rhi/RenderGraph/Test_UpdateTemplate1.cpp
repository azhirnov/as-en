...
namespace
{
	...
	static bool  UT1_CheckImageData (const ImageMemView &view, uint blockSize)
	{
	...
	static RenderCoro  UT1_ComputeTask (UT1_TestData& t)
	{
	...
	template <typename Ctx>
	static RenderCoro  UT1_CopyTask (UT1_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
		...
		t.result0 = ctx.ReadbackImage( t.img0, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<UT1_TestData &> t) -> InlineCoro<>
							{
								auto view = co_await readRes;
								t->isOK_0 = UT1_CheckImageData( view, 8 );
							});
		t.result1 = ctx.ReadbackImage( t.img1, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<UT1_TestData &> t) -> InlineCoro<>
							{
								auto view = co_await readRes;
								t->isOK_1 = UT1_CheckImageData( view, 4 );
							});
		t.result2 = ctx.ReadbackImage( t.img2, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<UT1_TestData &> t) -> InlineCoro<>
							{
								auto view = co_await readRes;
								t->isOK_2 = UT1_CheckImageData( view, 16 );
							});
	...
	static bool  UpdateTemplate1Test ()
	{
...

		{
			...
			CHECK_ERR( updater.Set( t.ds0, EDescUpdateMode::UpdateTemplate ));
			updater.BindImage( UniformName{"un_OutImage"}, t.view0 );

			CHECK_ERR( updater.Set( t.ds1, EDescUpdateMode::UpdateTemplate ));
			updater.BindImage( UniformName{"un_OutImage"}, t.view1 );

			CHECK_ERR( updater.Set( t.ds2, EDescUpdateMode::UpdateTemplate ));
		...
		AsyncTask	task1	= batch.Task( UT1_ComputeTask<CompCtx>(t), {"Compute task"} )
									.Run();

		AsyncTask	task2	= batch.Task( UT1_CopyTask<CopyCtx>(t), {"Readback task"} )
...
RGTest::ECode  RGTest::Test_UpdateTemplate1 ()
{
	bool	result = true;

	RG_CHECK( UpdateTemplate1Test< RG::DirectCtx::Compute,   RG::DirectCtx::Transfer   >());
	RG_CHECK( UpdateTemplate1Test< RG::IndirectCtx::Compute, RG::IndirectCtx::Transfer >());
	RG_CHECK( UpdateTemplate1Test< RG::DirectCtx::Compute,   RG::IndirectCtx::Transfer >());
