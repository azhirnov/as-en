...
RGTest::ECode  RGTest::Test_FeatureSets ()
{
	...
	#if 0
	{
		...
		for (;;)
		{
			...
			switch_enum( fmt )
			{
				...
				case ESurfaceFormat::RGBA16F_Extended_sRGB_nonlinear :	TEST( _pipelines->FeatureSetSupported( FeatureSetName{"part.Surface_RGBA16F_Extended_sRGB_nonlinear"} ));break;
	...
	# endif
		...
		// see FeatureSetGen: GenMinRecursiveRayTracing() for recursion depth limit
		if ( _device.GetVExtensions().rayTracingPipeline and _device.GetVProperties().rayTracingPipelineProps.maxRayRecursionDepth >= 4 )
