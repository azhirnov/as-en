...
namespace
{
	...
	static RenderCoro  RT2_UploadTask (RT2_TestData& t)
	{
		...
		CHECK_CE( copy_ctx.UploadBuffer( t.indirectBuf,	0_b, Sizeof(ind_cmd), &ind_cmd, EStagingHeapType::Static ));
	...
	template <typename CtxTypes>
	static RenderCoro  RT2_RayTracingTask (RT2_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
	...
	static RenderCoro  RT2_CopyTask (RT2_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
		...
		t.result = ctx.ReadbackImage( t.img, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<RT2_TestData &> t) -> InlineCoro<>
							{
	...
	static bool  RayTracing2Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		t.batch	= rts.BeginCmdBatch( EQueueType::Graphics, 0, {"RayTracing2"} );
		CHECK_ERR( t.batch );

		AsyncTask	task1	= t.batch->Run( RT2_UploadTask<CtxTypes>(t),	 Tuple{},						{"Upload RTAS task"} );
		AsyncTask	task2	= t.batch->Run( RT2_RayTracingTask<CtxTypes>(t), Tuple{task1},					{"Ray tracing task"} );
		AsyncTask	task3	= t.batch->Run( RT2_CopyTask<CopyCtx>(t),		 Tuple{task2}, True{"Last"},	{"Readback task"} );
...
RGTest::ECode  RGTest::Test_RayTracing2 ()
{
	if ( _rtPipelines == null or
		 GraphicsScheduler().GetFeatureSet().accelerationStructureIndirectBuild != FeatureSet::EFeature::RequireTrue )
	{
