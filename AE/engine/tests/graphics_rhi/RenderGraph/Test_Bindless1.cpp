...
namespace
{
	...
	static RenderCoro  Bls1_ComputeTask (Bls1_TestData& t)
	{
	...
	static RenderCoro  Bls1_CopyTask (Bls1_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
		...
		t.result = ctx.ReadbackImage( t.img, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<Bls1_TestData &> t) -> InlineCoro<>
							{
	...
	static bool  Bindless1Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		auto&			rts			= GraphicsScheduler();
		auto&			res_mngr	= rts.GetResourceManager();
		Bls1_TestData	t;
		const auto		format		= EPixelFormat::RGBA8_UNorm;
		const uint		maxTexCount	= 1000;

		t.gfxAlloc	= res_mngr.CreateLinearGfxMemAllocator();
		t.imgCmp	= imageCmp;
		t.viewSize	= uint2{800, 600};
		t.texCount	= 500;	// must be <= maxTexCount
...

		{
			...
			CHECK_Eq( updater.ImageCount( UniformName{"un_Textures"} ), maxTexCount );
		...
		AsyncTask	task1	= batch->Run( Bls1_ComputeTask<CompCtx, CopyCtx>(t),	Tuple{},		 			{"Compute task"} );
		AsyncTask	task2	= batch->Run( Bls1_CopyTask<CopyCtx>(t),				Tuple{task1}, True{"Last"},	{"Readback task"} );
...
RGTest::ECode  RGTest::Test_Bindless1 ()
{
	if ( _blsPipelines == null )
	{
	...
	RG_CHECK( Bindless1Test< DirectCtx::Compute,   DirectCtx::Transfer   >( _blsPipelines, img_cmp.get() ));
	RG_CHECK( Bindless1Test< IndirectCtx::Compute, IndirectCtx::Transfer >( _blsPipelines, img_cmp.get() ));
