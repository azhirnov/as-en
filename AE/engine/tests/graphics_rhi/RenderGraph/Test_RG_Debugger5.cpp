...
namespace
{
	...
	static RenderCoro  Db5_UploadTask (Db5_TestData& t)
	{
	...
	template <typename CtxTypes>
	static RenderCoro  Db5_RayTracingTask (Db5_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
		...
		CHECK_CE( t.debugger.AllocForCompute( OUT dbg, copy_ctx, t.ppln, uint3{t.viewSize/2, 0} ));


		typename CtxTypes::Compute		ctx{ RenderCoro_Get(), copy_ctx.ReleaseCommandBuffer() };
	...
	static RenderCoro  Db5_CopyTask (Db5_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
...
34. 	imageStore( un_OutImage, ivec2(gl_GlobalInvocationID.xy), color );
	...
	static bool  Debugger5Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		AsyncTask	task1	= t.batch->Run( Db5_UploadTask<CtxTypes>(t),	 Tuple{},						{"Upload RTAS task"} );
		AsyncTask	task2	= t.batch->Run( Db5_RayTracingTask<CtxTypes>(t), Tuple{task1},					{"Ray tracing task"} );
		AsyncTask	task3	= t.batch->Run( Db5_CopyTask<CopyCtx>(t),		 Tuple{task2}, True{"Last"},	{"Readback task"} );
...
RGTest::ECode  RGTest::Test_Debugger5 ()
{
	if ( _dbgPipelines == null or _rqPipelines == null )
	{
