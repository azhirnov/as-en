...
namespace
{
	...
	static RenderCoro  RT1_UploadTask (RT1_TestData& t)
	{
		...
		RTSceneBuild	scene_build{ c_InstanceCount, Default };
		...
		switch_enum( copy_ctx.GetDevice().GetGraphicsAPI() )
		{
			...
			{
				StaticArray< RTSceneBuild::InstanceVk, c_InstanceCount >		inst_arr;
	...
	static bool  RayTracing1Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		auto	scene_sizes = res_mngr.GetRTSceneSizes( RTSceneBuild{ c_InstanceCount, Default });
		...
		auto	batch	= rts.BeginCmdBatch( EQueueType::Graphics, 0, {"RayTracing1"} );
		CHECK_ERR( batch );

		AsyncTask	task1	= batch->Run( RT1_UploadTask<CtxTypes>(t),		Tuple{},					{"Upload RTAS task"} );
		AsyncTask	task2	= batch->Run( RT1_RayTracingTask<CtxTypes>(t),	Tuple{task1},				{"Ray tracing task"} );
		AsyncTask	task3	= batch->Run( RT1_CopyTask<CopyCtx>(t),			Tuple{task2}, True{"Last"},	{"Readback task"} );
...
RGTest::ECode  RGTest::Test_RayTracing1 ()
{
