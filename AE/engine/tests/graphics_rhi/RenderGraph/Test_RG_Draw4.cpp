...
namespace
{
	...
	static RenderCoro  D4_DrawTask (D4_TestData& t, const uint mode)
	{
	...
	static bool  Draw4Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		AsyncTask	task1	= batch.Task( D4_DrawTask<CtxTypes>(t, 0u), {"Draw task1"} )
									.UseResource( t.img, EResourceState::ColorAttachment )
									.Run();
		AsyncTask	task2	= batch.Task( D4_DrawTask<CtxTypes>(t, 1u), {"Draw task2"} )
									.UseResource( t.img, EResourceState::ColorAttachment )
									.Run();

		AsyncTask	task3	= batch.Task( D4_CopyTask<CopyCtx>(t), {"Readback task"} )
