...
namespace
{
	ND_ static bool  TestBufferAllocation (GfxMemAllocatorPtr allocator, EMemoryType memType)
	{
		...
		auto	buf_id = res_mngr.CreateBuffer( BufferDesc{ 1_KiB, EBufferUsage::Storage }.SetMemory( memType ), "", allocator );
		CHECK_ERR( buf_id );
	...
	ND_ static bool  TestAllocators ()
	{
		auto&	res_mngr = GraphicsScheduler().GetResourceManager();
		{
			auto	alloc = res_mngr.CreateLinearGfxMemAllocator( 64_KiB );
			CHECK_ERR( alloc );
			CHECK_ERR( TestAllocators( alloc ));
		}{
			auto	alloc = res_mngr.CreateBlockGfxMemAllocator( 4_KiB, 64_KiB );
			CHECK_ERR( alloc );
			CHECK_ERR( TestAllocators( alloc ));
		}{
			auto	alloc = res_mngr.CreateUnifiedGfxMemAllocator( 64_KiB );
			CHECK_ERR( alloc );
			CHECK_ERR( TestAllocators( alloc ));
		}{
