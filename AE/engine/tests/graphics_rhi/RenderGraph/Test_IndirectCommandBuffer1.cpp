...
namespace
{
	...
	static RenderCoro  ICB1_DrawTask (ICB1_TestData& t)
	{
		...
		typename CtxType::Graphics	gctx{ RenderCoro_Get(), tctx.ReleaseCommandBuffer() };

		gctx.AccumBarriers()
			.ResourceBarrier( t.img, EResourceState::Invalidate, img_state )
			.ResourceBarrier( t.inputBuf, EResourceState::CopyDst, EResourceState::IndirectBuffer )
			.ResourceBarrier( t.preprocessBuf, EResourceState::Invalidate, EResourceState::IndirectBuffer );
		...
		// draw
		{
			...
			dctx.BindInitialPipeline( t.execSet );
	...
	static RenderCoro  ICB1_CopyTask (ICB1_TestData& t)
	{
		DeferExLock	lock {t.guard};
		CHECK_CE( lock.try_lock() );
		...
		t.result = ctx.ReadbackImage( t.img, Default ).IfFullyRead( t,
							[] (Promise<ImageMemView> readRes, CoSafe<ICB1_TestData &> t) -> InlineCoro<>
							{
	...
	static bool  ICB1Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		// create command layout
		{
			...
			builder.PipelineLayoutFrom( t.execSet );

			// must be first token
			builder.ExecutionSet();
		...
		// create buffers
		{
			...
			t.inputBuf		= res_mngr.CreateBuffer( BufferDesc{ Sizeof(t.cmdData), EBufferUsage::ShaderAddress | EBufferUsage::Transfer | EBufferUsage::Indirect }, "input buffer" );
		...
		auto		batch	= rts.BeginCmdBatch( EQueueType::Graphics, 0, {"ICB1.Draw"} );
		CHECK_ERR( batch );

		AsyncTask	task1	= batch->Run( ICB1_DrawTask<CtxType>(t), Tuple{},						{"Draw task"} );
		AsyncTask	task2	= batch->Run( ICB1_CopyTask<CopyCtx>(t), Tuple{task1}, True{"Last"},	{"Readback task"} );
...
RGTest::ECode  RGTest::Test_IndirectCommandBuffer1 ()
{
	...
	RG_CHECK( ICB1Test< DirectCtx,   DirectCtx::Transfer   >( _icbPipelines, img_cmp.get() ));
	RG_CHECK( ICB1Test< DirectCtx,   IndirectCtx::Transfer >( _icbPipelines, img_cmp.get() ));

	RG_CHECK( ICB1Test< IndirectCtx, DirectCtx::Transfer   >( _icbPipelines, img_cmp.get() ));
	RG_CHECK( ICB1Test< IndirectCtx, IndirectCtx::Transfer >( _icbPipelines, img_cmp.get() ));
