...
namespace
{
	...
	static bool  RayTracing2Test (RenderTechPipelinesPtr renderTech, ImageComparator* imageCmp)
	{
		...
		auto	batch	= rts.BeginCmdBatch( EQueueType::Graphics, 0, {"RayTracing2"} );
		CHECK_ERR( batch );

		AsyncTask	task1	= batch->Run( RT2_UploadTask<CtxTypes>(t),		Tuple{},					{"Upload RTAS task"} );
		AsyncTask	task2	= batch->Run( RT2_RayTracingTask<CtxTypes>(t),	Tuple{task1},				{"Ray tracing task"} );
		AsyncTask	task3	= batch->Run( RT2_CopyTask<CopyCtx>(t),			Tuple{task2}, True{"Last"},	{"Readback task"} );
