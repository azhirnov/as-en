...
{
	vec2	point = vec2(gl_GlobalInvocationID.xy + uvec2(0,10));

	float	arr[5];
	arr[0] = point.x;
	arr[1] = point.y;
	arr[2] = point.x + point.y;
	arr[3] = point.x * point.y;

	color.rgb = point.xyx;
	color.a = arr[ gl_GlobalInvocationID.x % arr.length() ];
}
...
extern bool ShaderTrace_Test16 (TestDevice& vulkan)
{
	...
	CHECK_ERR( vulkan.TestDebugTraceOutput( {comp_shader}, "ShaderTrace_Test16.txt" ));
