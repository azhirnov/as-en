...
namespace
{
    ...
    static const auto   c_Codec = EVideoCodec::H264;

    static void  EnableHWAcceleration (IVideoEncoder::Config &cfg)
    {
    ...
    static void  FFmpeg_Test1 ()
    {
        ...
        // decode
        {
            ...
            for (uint i = 0; i < frame_count-2; ++i)
            {
                const RGBA8u    col {RainbowWrap( i * 0.2f )};
                for (auto& c : pixels) { c = col; }
    ...
    static void  FFmpeg_Test3 ()
    {
        ...
        // decode
        {
            ...
            IVideoDecoder::ImageMemViewArr  view_arr;
            Allocator_t                     alloc;
            ...
            for (uint i = 0; i < frame_count-2; ++i)
            {
                TEST( dec->GetNextFrame( INOUT view_arr, OUT fi ));
                //TEST( fi.frameIdx == i );
            }
