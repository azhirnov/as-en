...
float   ApplySRGBCurve (float  x)		{ return Select( x < 0.0031308,  (12.92 * x),  (1.055 * Pow(x, 1.0/2.4) - 0.055) ); }
float3  ApplySRGBCurve (float3 v)		{ return Select( Less( v, float3(0.0031308)),  (12.92 * v),  (1.055 * Pow( v, float3(1.0/2.4) ) - 0.055) ); }
float4  ApplySRGBCurve (float4 v)		{ return float4( ApplySRGBCurve( v.rgb ), v.a ); }

// Approximately pow(x, 2.2)
float   RemoveSRGBCurve (float  x)		{ return Select( (x < 0.04045),  (x / 12.92),  Pow( (x + 0.055) / 1.055, 2.4 )); }
float3  RemoveSRGBCurve (float3 v)		{ return Select( Less( v, float3(0.04045)),  (v / 12.92),  Pow( (v + 0.055) / 1.055, float3(2.4) )); }
...
float   ApplySRGBCurve_Fast (float  x)	{ return Select( x < 0.0031308,  (12.92 * x),  (1.13005 * Sqrt(x - 0.00228) - 0.13448 * x + 0.005719) ); }
float3  ApplySRGBCurve_Fast (float3 v)	{ return Select( Less( v, float3(0.0031308)),  (12.92 * v),  (1.13005 * Sqrt(v - 0.00228) - 0.13448 * v + 0.005719) ); }
float4  ApplySRGBCurve_Fast (float4 v)	{ return float4( ApplySRGBCurve_Fast( v.rgb ), v.a ); }

float   RemoveSRGBCurve_Fast (float  x)	{ return Select( x < 0.04045,  (x / 12.92),  (-7.43605 * x - 31.24297 * Sqrt(-0.53792 * x + 1.279924) + 35.34864) ); }
float3  RemoveSRGBCurve_Fast (float3 v)	{ return Select( Less( v, float3(0.04045)),  (v / 12.92),  (-7.43605 * v - 31.24297 * Sqrt(-0.53792 * v + 1.279924) + 35.34864) ); }
...
float   ApplyREC709Curve (float  x)		{ return Select( x < 0.0181,  (4.5 * x),  (1.0993 * Pow(x, 0.45) - 0.0993) ); }
float3  ApplyREC709Curve (float3 v)		{ return Select( Less( v, float3(0.0181)),  (4.5 * v),  (1.0993 * Pow( v, float3(0.45) ) - 0.0993) ); }
float4  ApplyREC709Curve (float4 v)		{ return float4( ApplyREC709Curve( v.rgb ), v.a ); }

float   RemoveREC709Curve (float  x)	{ return Select( x < 0.08145,  (x / 4.5),  Pow((x + 0.0993) / 1.0993, 1.0/0.45) ); }
float3  RemoveREC709Curve (float3 v)	{ return Select( Less( v, float3(0.08145)),  (v / 4.5),  Pow( (v + 0.0993) / 1.0993, float3(1.0/0.45) )); }
...
// from https://github.com/microsoft/DirectX-Graphics-Samples/blob/master/MiniEngine/Core/Shaders/ShaderUtility.hlsli
...
float3  ApplyDisplayProfile (const float3 color, const int displayFormat)
{
	switch ( displayFormat )
	{
		...
		case COLOR_FORMAT_Rec2020_FULL :	return ApplyREC2084Curve( color );
		case COLOR_FORMAT_Rec2020_LIMITED :	return RGBFullToLimited10bit( ApplyREC2084Curve( color ));
	};
}
