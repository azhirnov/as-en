...
namespace AE::Threading
{
	...
	// types
	protected:
		using AsyncTaskImpl = AE::_Coro_::AsyncTaskImpl;


	// interface
	public:
		// Before task execution in current thread.
		virtual void  Begin (const AsyncTaskImpl &)						__NE___	= 0;

		// After task execution in current thread.
		virtual void  End (const AsyncTaskImpl &)							__NE___	= 0;

		// Task added to the queue at first time or rerunning, when used 'Continue()'.
		virtual void  Enqueue (const AsyncTaskImpl &)						__NE___	= 0;
