	...
	void  MaliProfilerServer::_UpdateMaliProfiler () __NE___
	{
		...
		if ( is_sent )
		{
			...
			for (auto it = _prof.counters.begin(); it != _prof.counters.end();)
			{
				...
				auto	msg = _msgProducer->CreateMsg< CSMsg_MaliProf_Sample >( SizeOf<KeyVal> * (count-1) );
				if ( not msg )
					break;
...
=================================================
*/
	void  MaliProfilerClient::Sample (OUT Counters_t &result, INOUT float &invdt) __NE___
	{
		result.clear();
		invdt = 0.f;
