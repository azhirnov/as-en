...
=================================================
*/
	void  PowerVRProfiler::Sample (OUT Counters_t &result, INOUT float &invdt) C_NE___
	{
		result.clear();
...
*/
	void  PowerVRProfiler::ReadTimingData (OUT TimeScopeArr_t &result) C_NE___
	{
		...
		Msg::ProfPVR_GetTiming				msg;
		RC<Msg::ProfPVR_GetTiming_Response>	res;

		CHECK_ERRV( _impl->dev.SendAndWait( msg, OUT res ));
		result = RVRef(res->timings);
	}
		...
		if ( count > 0 and packets != null )
		{
			...
			for (uint i = 0; i < count; ++i)
			{
				...
				switch_enum( packet.eEventType )
				{
					...
					case ePVRScopeEventTAEnd :			pass = EPass::Tiler;		break;
					case ePVRScopeEvent3DEnd :			pass = EPass::Renderer;		break;
...
=================================================
*/
	void  PowerVRProfiler::Sample (OUT Counters_t &outCounters, INOUT float &invdt) C_NE___
	{
		outCounters.clear();
		Unused( invdt );	// keep current
...
namespace AE::Profiler
{
	...
	void  PowerVRProfiler::Sample (OUT Counters_t &result, INOUT float &invdt)	C_NE___	{ if (_impl) return _impl->client->Sample( OUT result, INOUT invdt ); }
