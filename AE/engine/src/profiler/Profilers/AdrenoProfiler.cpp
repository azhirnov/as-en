...
namespace AE::Profiler
{
	...
	void  AdrenoProfiler::Deinitialize ()								__NE___	{ _impl.reset(); }
...
=================================================
*/
	void  AdrenoProfiler::Sample (OUT Counters_t &result, INOUT float &invdt) C_NE___
	{
		result.clear();
...
*/
	ND_ static Pair<uint, uint>  UnpackGroupAndSelector_A5xx (const AdrenoProfiler::ECounter c) __NE___
	{
		...
		if_likely( c < ECounter::_Count )
		{
			...
			// PC
				0x02'10,	// PC_Instances
				0x02'11,	// PC_VPCPrimitives
				0x02'12,	// PC_DeadPrim
				0x02'13,	// PC_LivePrim
				0x02'14,	// PC_VertexHits
			...
			// Shader/Streaming Processor
				0x0A'01,	// SSP_ALUcy
				0x0A'02,	// SSP_EFUcy
				0x0A'23,	// SSP_VS_EFUInst
				0x0A'24,	// SSP_VS_FullALUInst
				0x0A'25,	// SSP_VS_HalfALUInst
				0x0A'28,	// SSP_FS_EFUInst
				0x0A'29,	// SSP_FS_FullALUInst
				0x0A'30,	// SSP_FS_HalfALUInst
				0x0A'2F,	// SSP_L2Read
				0x0A'30,	// SSP_L2Write
			...
			// Cache and Compression Unit
				...
				0x18'16,	// CCU_2dReadReq
				0x18'17,	// CCU_2dWriteReq
			...
			// CMP
				0x1A'16,	// CMP_2dReadData
				0x1A'17,	// CMP_2dWriteData
			};
	...
	ND_ static Pair<uint, uint>  UnpackGroupAndSelector_A6xx (const AdrenoProfiler::ECounter c) __NE___
	{
		...
		if_likely( c < ECounter::_Count )
		{
			...
			// Shader/Streaming Processor
				0x0A'01,	// SSP_ALUcy
				0x0A'02,	// SSP_EFUcy
				0x0A'22,	// SSP_VS_EFUInst
				0x0A'23,	// SSP_VS_FullALUInst
				0x0A'24,	// SSP_VS_HalfALUInst
				0x0A'27,	// SSP_FS_EFUInst
				0x0A'28,	// SSP_FS_FullALUInst
				0x0A'29,	// SSP_FS_HalfALUInst
				0x0A'2E,	// SSP_L2Read
				0x0A'2F,	// SSP_L2Write
			...
			// Cache and Compression Unit
				...
				0x18'09,	// CCU_GMemRead
				0x18'0A,	// CCU_GMemWrite
				0x18'1B,	// CCU_2dReadReq
				0x18'1C,	// CCU_2dWriteReq
			...
			// CMP
				0x1A'1C,	// CMP_2dReadData
				0x1A'1D,	// CMP_2dWriteData
			};
			StaticAssert( CountOf(values) == uint(ECounter::_Count) );
			return Unpack( values[ uint(c) ]);
		}
...
*/
	bool  AdrenoProfiler::Impl::QueryCounters () __NE___
	{
		...
		int	err = ::ioctl( _gpuDevice, ADRENO_IOCTL_COUNTER_READ, INOUT &read );
		if ( err == -1 )
		{
			UNIX_CHECK_DEV( "failed to read Adreno counters: " );
			return false;
		}
...
=================================================
*/
	void  AdrenoProfiler::Impl::Print () C_NE___
	{
	  #ifdef AE_ENABLE_LOGS
		const uint	max_group	= 0x22;
		const uint	max_counter	= 255;
		String		str			= "Adreno GPU info";

		str << "\nseries:  ";
		switch_enum( _hwInfo.series )
		{
			case EGPUSeries::A5xx :		str << "A5xx";	break;
			case EGPUSeries::A6xx :		str << "A6xx";	break;
			case EGPUSeries::A7xx :		str << "A7xx";	break;
			case EGPUSeries::Unknown :
			default :					str << "unknown"; break;
		}
		switch_end
		str << "\ngmem:    " << ToString( _hwInfo.gmemSize );
		...
		for (uint g = 0; g <= max_group; ++g)
		{
			...
			for (uint c = 0; c <= max_counter; ++c)
			{
				...
				if ( ::ioctl( _gpuDevice, ADRENO_IOCTL_COUNTER_GET, INOUT &cnt ) != -1 )
				{
					str << "| " << ToString(g) << ", " << ToString(c) << " | " << GetCounterName( _hwInfo.series, g, c ) << " |\n";

					// deactivate
					::adreno_counter_put	put = {};
					put.group_id			= g;
					put.countable_selector	= c;
					::ioctl( _gpuDevice, ADRENO_IOCTL_COUNTER_PUT, INOUT &put );
				}
			}
		}
...
*/
	bool  AdrenoProfiler::Initialize (const ECounterSet &counterSet) __NE___
	{
		...
		// get ID
		{
			...
			if_unlikely( ::ioctl( impl->_gpuDevice, ADRENO_IOCTL_DEVICE_GET_PROPERTY, INOUT &prop ) == -1 )
				return false;
		...
		{{
			...
			if_unlikely( ::ioctl( impl->_gpuDevice, ADRENO_IOCTL_COUNTER_GET, INOUT &cnt ) == -1 )
			{
...
=================================================
*/
	void  AdrenoProfiler::Sample (OUT Counters_t &outCounters, INOUT float &invdt) C_NE___
	{
		outCounters.clear();
		Unused( invdt );	// keep current
...
namespace AE::Profiler
{
	...
	void  AdrenoProfiler::Sample (OUT Counters_t &result, INOUT float &invdt)	C_NE___	{ if (_impl) return _impl->client->Sample( OUT result, INOUT invdt ); }
