...
namespace AE::Profiler
{
	...
	public:
		...
		enum class EPass : ubyte
		{
			...
			Compute,
			Tiler,			// binning & VS
			Renderer,		// rasterization & FS
			Transfer,		// 2D, image blit, image/buffer copy
			RayTracing,
			RTASBuild,		// SHG
		};
	...
	public:
		...
		ND_ ECounterSet  EnabledCounterSet ()							C_NE___;
			...
			void  Tick ()												C_NE___;
			void  Sample (OUT Counters_t &, INOUT float &invdt)			C_NE___;	// use once per second
