...
namespace AE::Base
{
	...
	public:
		NdCx__ static usize  Resize (usize count, bool allowReserve) __NE___
		{
			return Max( (allowReserve ? count * 2 : count), _MinSize );
		}

		NdCx__ static usize  Resize (Bytes elementSizeOf, usize count, bool allowReserve) __NE___
		{
			...
			ASSERT_Cx( result >= new_count );
			return result;
		}

		NdCx__ static Bytes  ResizeInBytes (Bytes newSize) __NE___
		{
