...
namespace AE::Serializing
{
    ...
    template <typename T>
    bool  Deserializer::_Deserialize (INOUT TBytes<T> &value)
    {
        T       val = {};
    ...
    bool  Deserializer::_Deserialize (INOUT ArrayView<T> &arr)
    {
        ...
        else
        {
            for (uint i = 0; res & (i < count); ++i)
            {
                PlacementNew<T>( OUT dst + i );         // nothrow
    ...
    bool  Deserializer::_Deserialize (INOUT BasicStringView<T> &str)
    {
        ...
        T*  dst = Cast<T>( allocator->Allocate( SizeOf<T> * (len+1), AlignOf<T> ));
        if_unlikely( dst == null )
            return false;
