...
namespace AE::Serializing
{
	...
	public:
		...
		~Serializer ()								__NE___	{ Unused( _FlushBits() ); }
	...
	private:
		...
		template <typename T, usize S>		ND_ bool  _Serialize (const FixedArray<T,S> &arr)	__NE___	{ return _Serialize(ArrayView<T>{arr}); }
		...
		template <typename K, typename V, typename H, typename E, typename A>	ND_ bool  _Serialize (const FlatHashMap<K,V,H,E,A> &map)	__NE___	{ return _SerializeMap<K,V>( map ); }
		#endif
		template <typename K, typename V, typename H, typename E, typename A>	ND_ bool  _Serialize (const HashMap<K,V,H,E,A> &map)		__NE___	{ return _SerializeMap<K,V>( map ); }
		template <typename K, typename V, typename H, typename E, typename A>	ND_ bool  _Serialize (const HashMultiMap<K,V,H,E,A> &map)	__NE___	{ return _SerializeMap<K,V>( map ); }
		template <typename K, typename V, usize S>								ND_ bool  _Serialize (const FixedMap<K,V,S> &map)			__NE___	{ return _SerializeMap<K,V>( map ); }
		template <typename K, typename V, typename MapType>						ND_ bool  _SerializeMap (const MapType &)					__NE___;

		#ifdef AE_ENABLE_ABSEIL
		template <typename T, typename H, typename E, typename A>				ND_ bool  _Serialize (const FlatHashSet<T,H,E,A> &set)		__NE___	{ return _SerializeSet<T>( set ); }
		#endif
		template <typename T, typename H, typename E, typename A>				ND_ bool  _Serialize (const HashSet<T,H,E,A> &set)			__NE___	{ return _SerializeSet<T>( set ); }
		template <typename T, usize S>											ND_ bool  _Serialize (const FixedSet<T,S> &set)				__NE___	{ return _SerializeSet<T>( set ); }
		template <typename T, typename SetType>									ND_ bool  _SerializeSet (const SetType &)					__NE___;
