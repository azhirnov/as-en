...
namespace AE::App
{
	...
	public:
		...
		using MonitorsView_t	= ArrayView< Monitor, Monitor::ID >;
	...
	public:
		...
		// Create new window.
		//   Android supports only one window (TODO: desktop mode).
		...
		//   Thread safe: main thread only
		//
		ND_ virtual WindowPtr  CreateVRSession (WndListenerPtr,
												IInputActions*			dstIA	= null,
												IVRSession::EDeviceType	type	= Default)	__NE___ = 0;
		...
		//   Thread safe: main thread only
		//
		ND_ virtual MonitorsView_t  GetMonitors (bool update = false)						__NE___	= 0;
		ND_ virtual MonitorsView_t  GetCachedMonitors ()									C_NE___	= 0;


		// Returns monitor ID for position on desktop.
		//   Thread safe: main thread only
		//
		ND_ virtual Monitor::ID  GetMonitor (int2 pos)										C_NE___ = 0;
		ND_ virtual Monitor::ID  GetMonitorFromNative (Monitor::NativeMonitor_t)			C_NE___ = 0;
		...
		// Returns new screen capture object.
		//   Thread safe: main thread only
		//
		ND_ virtual RC<IScreenCapture>  StartScreenCapture (const IScreenCapture::Config &)	__NE___ { return Default; }


		// TODO
		//   Thread safe: yes
		//
		ND_ virtual Unique<ISendInput>  CreateInputSender ()								__NE___	{ return Default; }
	...
	public:
		...
		ND_ Promise<RC<IScreenCapture>>	StartScreenCapture (const IScreenCapture::Config &) __NE___;
	};
