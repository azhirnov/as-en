...
#define RG_INHERIT_BARRIERS \
	...
	public: \
		...
		using BaseDrawContext_t			= IDrawContextVk;
		using BaseTransferContext_t		= ITransferContextVk;
		using BaseComputeContext_t		= IComputeContextVk;
		using BaseGraphicsContext_t		= IGraphicsContextVk;
		using BaseRayTracingContext_t	= IRayTracingContextVk;
		using BaseASBuildContext_t		= IASBuildContextVk;
		using BaseVideoDecodeContext_t	= IVideoDecodeContextVk;
		using BaseVideoEncodeContext_t	= IVideoEncodeContextVk;
	#else
		using BaseDrawContext_t			= IDrawContext;
		using BaseTransferContext_t		= ITransferContext;
		using BaseComputeContext_t		= IComputeContext;
		using BaseGraphicsContext_t		= IGraphicsContext;
		using BaseRayTracingContext_t	= IRayTracingContext;
		using BaseASBuildContext_t		= IASBuildContext;
		using BaseVideoDecodeContext_t	= IVideoDecodeContext;
		using BaseVideoEncodeContext_t	= IVideoEncodeContext;
	#endif
	...
	class TransferContext final : public BaseTransferContext_t
	{
	// types
	public:
	...
	public:
		...
		void  UploadImage (ImageID, const UploadImageDesc &desc, Bytes maxSize, OUT ImageMemView &memView)		__Th_OV;
		void  UploadImage (VideoImageID, const UploadImageDesc &desc, Bytes maxSize, OUT ImageMemView &memView)	__Th_OV;
	...
	template <typename BaseCtx>
	class ComputeContext final : public BaseComputeContext_t
	{
	// types
	public:
	...
	template <typename BaseCtx>
	class DrawContext final : public BaseDrawContext_t
	{
	// types
	public:
	...
	// dynamic states //
		void  SetViewport (const Viewport &viewport)																		__Th_OV	{ return _ctx.SetViewport( viewport ); }
		void  SetViewports (ArrayView<Viewport> viewports)																	__Th_OV	{ return _ctx.SetViewports( viewports ); }
	  ...
	  #if defined(AE_ENABLE_VULKAN) or defined(AE_ENABLE_REMOTE_GRAPHICS)
		void  SetDepthBounds (float minDepthBounds, float maxDepthBounds)													__Th_OV	{ return _ctx.SetDepthBounds( minDepthBounds, maxDepthBounds ); }
		void  SetStencilCompareMask (uint compareMask)																		__Th_OV	{ return _ctx.SetStencilCompareMask( compareMask ); }
		void  SetStencilCompareMask (uint frontCompareMask, uint backCompareMask)											__Th_OV	{ return _ctx.SetStencilCompareMask( frontCompareMask, backCompareMask ); }
	...
	template <typename BaseCtx>
	class GraphicsContext final : public BaseGraphicsContext_t
	{
	// types
	public:
	...
	template <typename BaseCtx>
	class RayTracingContext final : public BaseRayTracingContext_t
	{
	// types
	public:
	...
	template <typename BaseCtx>
	class ASBuildContext final : public BaseASBuildContext_t
	{
	// types
	public:
	...
	public:
		...
		// warning: state tracking is not supported for 'DeviceAddress'
		void  BuildIndirect (const RTGeometryBuild &, RTGeometryID dst, DeviceAddress indirectBuffer, Bytes indirectStride)	__Th_OV;
	...
	template <typename BaseCtx>
	class VideoDecodeContext final : public BaseVideoDecodeContext_t
	{
	// types
	public:
	...
	template <typename BaseCtx>
	class VideoEncodeContext final : public BaseVideoEncodeContext_t
	{
	// types
	public:
...
//-----------------------------------------------------------------------------
	...
	template <typename C>
	void  TransferContext<C>::UploadImage (ImageID image, const UploadImageDesc &uploadDesc, Bytes maxSize, OUT ImageMemView &memView) __Th___
	{
		ResourceState( image, EResourceState::CopyDst );

		_ctx.CommitBarriers();
		_ctx.UploadImage( image, uploadDesc, maxSize, OUT memView );
	}

	template <typename C>
	void  TransferContext<C>::UploadImage (VideoImageID image, const UploadImageDesc &uploadDesc, Bytes maxSize, OUT ImageMemView &memView) __Th___
	{
		ResourceState( image, EResourceState::CopyDst );
...
//-----------------------------------------------------------------------------
	...
	template <typename C>
	void  ASBuildContext<C>::BuildIndirect (const RTGeometryBuild &cmd, RTGeometryID dst, DeviceAddress indirectBuffer, Bytes indirectStride) __Th___
	{
		_RTGeometryBuildBarriers( cmd );
		ResourceState( dst, EResourceState::BuildRTAS_ReadWrite );
		_ctx.CommitBarriers();
		_ctx.BuildIndirect( cmd, dst, indirectBuffer, indirectStride );
	}

	template <typename C>
	void  ASBuildContext<C>::BuildIndirect (const RTSceneBuild &cmd, RTSceneID dst, DeviceAddress indirectBuffer) __Th___
	{
		_RTSceneBuildBarriers( cmd );
		ResourceState( dst, EResourceState::BuildRTAS_ReadWrite );
		_ctx.CommitBarriers();
		_ctx.BuildIndirect( cmd, dst, indirectBuffer );
	}

	template <typename C>
	void  ASBuildContext<C>::BuildClusterIndirect (const RTClusterBuild &cmd) __Th___
	{
		// TODO

		_ctx.CommitBarriers();
		_ctx.BuildClusterIndirect( cmd );
	}

	template <typename C>
	void  ASBuildContext<C>::BuildPartitionedIndirect (const RTPartitionedSceneBuild &cmd) __Th___
	{
		using BufferWithOffset = RTPartitionedSceneBuild::BufferWithOffset;

		for (auto& id : cmd.uniqueGeoms)
			ResourceState( id, EResourceState::BuildRTAS_Read );

		if ( auto* buf = UnionGet<BufferWithOffset>( cmd.scratch ))
			ResourceState( buf->id, EResourceState::BuildRTAS_ScratchBuffer );

		if ( auto* buf = UnionGet<BufferWithOffset>( cmd.srcInfos ))
			ResourceState( buf->id, EResourceState::BuildRTAS_IndirectBuffer );

		if ( auto* buf = UnionGet<BufferWithOffset>( cmd.srcInfosCount ))
			ResourceState( buf->id, EResourceState::BuildRTAS_IndirectBuffer );

		_ctx.CommitBarriers();
		_ctx.BuildPartitionedIndirect( cmd );
	}
	...
	template <typename C>
	void  ASBuildContext<C>::SerializeToMemory (RTGeometryID src, DeviceAddress dst) __Th___
	{
		ResourceState( src, EResourceState::CopyRTAS_Read );
		_ctx.CommitBarriers();
	...
	template <typename C>
	void  ASBuildContext<C>::SerializeToMemory (RTSceneID src, DeviceAddress dst) __Th___
	{
		ResourceState( src, EResourceState::CopyRTAS_Read );
		_ctx.CommitBarriers();
		_ctx.SerializeToMemory( src, dst );
	...
	template <typename C>
	void  ASBuildContext<C>::DeserializeFromMemory (DeviceAddress src, RTGeometryID dst) __Th___
	{
		ResourceState( dst, EResourceState::CopyRTAS_Write );
		_ctx.CommitBarriers();
	...
	template <typename C>
	void  ASBuildContext<C>::DeserializeFromMemory (DeviceAddress src, RTSceneID dst) __Th___
	{
		ResourceState( dst, EResourceState::CopyRTAS_Write );
		_ctx.CommitBarriers();
		_ctx.DeserializeFromMemory( src, dst );
