...
=================================================
*/
	bool  EPixelFormat_GetPlaneInfo (EPixelFormat fmt, EImageAspect aspect, OUT EPixelFormat &planeFormat, OUT POTVec2 &dimScale) __NE___
	{
		ASSERT( EPixelFormat_isXChromaSubsampled( fmt ));

		const POTVec2	_1_1 = POTVec2::c_1_1();
		const POTVec2	_2_1 = POTVec2::c_2_1();
		const POTVec2	_2_2 = POTVec2::c_2_2();
		...
		switch ( fmt )
		{
			case EPixelFormat::G8_B8_R8_420_UNorm :
			{
				...
				switch ( aspect ) {
					case EImageAspect::Plane_0 :	dimScale = _1_1;	return true;
					case EImageAspect::Plane_1 :
					case EImageAspect::Plane_2 :	dimScale = _2_2;	return true;
				}
				break;
			}
			case EPixelFormat::G8_B8R8_420_UNorm :
			...
			case EPixelFormat::G8_B8_R8_422_UNorm :
			{
				...
				switch ( aspect ) {
					case EImageAspect::Plane_0 :	dimScale = _1_1;	return true;
					case EImageAspect::Plane_1 :
					case EImageAspect::Plane_2 :	dimScale = _2_1;	return true;
				}
				break;
			}
			case EPixelFormat::G8_B8R8_422_UNorm :
			...
			case EPixelFormat::G10x6_B10x6_R10x6_444_UNorm :
			{
				...
				switch ( aspect ) {
					case EImageAspect::Plane_0 :
					case EImageAspect::Plane_1 :
					case EImageAspect::Plane_2 :	dimScale = _1_1;	return true;
				}
				break;
			}
			case EPixelFormat::G12x4_B12x4_R12x4_420_UNorm :
...
/*
=================================================
	EPixelFormat_IsCompatible
=================================================
*/
	bool  EPixelFormat_IsCompatible (const EPixelFormat aFmt, const EPixelFormat bFmt) __NE___
	{
		if ( aFmt == bFmt )
			return true;

		auto&	a = EPixelFormat_GetInfo( aFmt );
		auto&	b = EPixelFormat_GetInfo( bFmt );

		// Vulkan docs '49.1.7. Format Compatibility Classes':
		//  "Uncompressed color formats are compatible with each other if they occupy the same number of bits per texel block."
		if ( a.IsColor() and b.IsColor() and not a.IsCompressed() and not b.IsCompressed() )
		{
			return	a.bitsPerBlock == b.bitsPerBlock	and
					All( a.blockDim == b.blockDim );
		}

		// Vulkan docs:
		//  "Compressed color formats are compatible with each other if the only difference between them is
		//   the numeric format of the uncompressed pixels."
		if ( a.IsCompressed() and b.IsCompressed() )
		{
			const auto	CompressionType = [] (EPixelFormat fmt)
			{{
				switch ( fmt )
				{
					case EPixelFormat::BC1_RGB8_UNorm :
					case EPixelFormat::BC1_sRGB8 :			return EPixelFormat::BC1_RGB8_UNorm;

					case EPixelFormat::BC1_RGB8_A1_UNorm :
					case EPixelFormat::BC1_sRGB8_A1 :		return EPixelFormat::BC1_RGB8_A1_UNorm;

					case EPixelFormat::BC2_RGBA8_UNorm :
					case EPixelFormat::BC2_sRGB8 :			return EPixelFormat::BC2_RGBA8_UNorm;

					case EPixelFormat::BC3_RGBA8_UNorm :
					case EPixelFormat::BC3_sRGB8 :			return EPixelFormat::BC3_RGBA8_UNorm;

					case EPixelFormat::BC4_R8_SNorm :
					case EPixelFormat::BC4_R8_UNorm :		return EPixelFormat::BC4_R8_UNorm;
					...
					case EPixelFormat::BC6H_RGB16UF :		return EPixelFormat::BC6H_RGB16F;

					case EPixelFormat::BC7_RGBA8_UNorm :
					case EPixelFormat::BC7_sRGB8_A8 :		return EPixelFormat::BC7_RGBA8_UNorm;

					case EPixelFormat::ETC2_RGB8_UNorm :
					case EPixelFormat::ETC2_sRGB8 :			return EPixelFormat::ETC2_RGB8_UNorm;

					case EPixelFormat::ETC2_RGB8_A1_UNorm :
					case EPixelFormat::ETC2_sRGB8_A1 :		return EPixelFormat::ETC2_RGB8_A1_UNorm;

					case EPixelFormat::ETC2_RGBA8_UNorm :
					case EPixelFormat::ETC2_sRGB8_A8 :		return EPixelFormat::ETC2_RGBA8_UNorm;
				...
				if ( EPixelFormat_IsASTC( fmt ))
					return EPixelFormat::ASTC_RGBA8_4x4;

				return fmt;
			}};

			return	CompressionType( aFmt )	== CompressionType( bFmt )	and
					All( a.blockDim == b.blockDim );
		}

		// Vulkan docs:
		//  "Each depth/stencil format is only compatible with itself."
		if ( a.HasDepthOrStencil() or b.HasDepthOrStencil() )
			return false;
...
/*
=================================================
	EPixelFormat_IsBlitSupported
=================================================
*/
	bool  EPixelFormat_IsBlitSupported (EPixelFormat src, EPixelFormat dst, EBlitFilter blitFilter) __NE___
	{
		const auto &	src_fmt	= EPixelFormat_GetInfo( src );
		const auto &	dst_fmt	= EPixelFormat_GetInfo( dst );
		...
		{
			if ( src != dst							or
				 blitFilter != EBlitFilter::Nearest )
				return false;
		}
...
/*
=================================================
	EPixelFormat_GetCopyGranularity
----
	returns 'false' if not compatible
=================================================
*/
	bool  EPixelFormat_GetCopyGranularity (const EPixelFormat src, OUT uint2 &srcBlockDim,
										   const EPixelFormat dst, OUT uint2 &dstBlockDim) __NE___
	{
		...
		{
			srcBlockDim	= a.TexBlockDim();
			dstBlockDim	= b.TexBlockDim();
			return true;
		}
...
/*
=================================================
	EPixelFormat_IsCopySupported
=================================================
*/
	bool  EPixelFormat_IsCopySupported (EPixelFormat src, EPixelFormat dst) __NE___
	{
		if ( src == dst )
			return true;

		uint2	a, b;
		return EPixelFormat_GetCopyGranularity( src, OUT a, dst, OUT b ) and All( a == b );
	}

	bool  EPixelFormat_IsCopySupported (const EPixelFormat src, const uint2 &srcDim,
										const EPixelFormat dst, const uint2 &dstDim) __NE___
	{
		if ( src == dst and All( srcDim == dstDim ))
			return true;

		uint2	src_gran;
		uint2	dst_gran;

		if ( EPixelFormat_GetCopyGranularity( src, OUT src_gran, dst, OUT dst_gran ))
		{
			uint2	s0 = srcDim / src_gran;
			uint2	s1 = dstDim / dst_gran;

			return	All( s0 == s1 )					and
					All( srcDim - s0 == uint2{0} )	and
					All( dstDim - s1 == uint2{0} );
		}
		return false;
	}
//-----------------------------------------------------------------------------
