...
*/
	void  PPLNPACK::_Destroy (ResourceManager &resMngr) __NE___
	{
		...
		// Framebuffers may keep reference to the render pass to a long time, but never use it.
		for (auto& [name, id] : _renderPassRefs->compatMap) {
			Unused( resMngr.ImmediatelyRelease2( INOUT id ));
		}
...
*/
	bool  PPLNPACK::_Create (ResourceManager &resMngr, const PipelinePackDesc &desc, PipelinePackID selfId) __Th___
	{
		...
		_unsupportedFS	.CreateOrThrow( StdAlloc_t< FeatureSetName::Optimized_t											 >{ _allocator.get() });
		_renTechMap		.CreateOrThrow( StdAlloc_t< Pair< const RenderTechName::Optimized_t,	ushort					>>{ _allocator.get() });
		_pplnTemplMap	.CreateOrThrow( StdAlloc_t< Pair< const PipelineTmplName::Optimized_t,	PipelineTemplUID		>>{ _allocator.get() });
		_dsLayoutMap	.CreateOrThrow( StdAlloc_t< Pair< const DescriptorSetName::Optimized_t,	DescriptorSetLayoutID	>>{ _allocator.get() });
		_samplerRefs	.CreateOrThrow( StdAlloc_t< Pair< const SamplerName::Optimized_t,		SamplerID				>>{ _allocator.get() });
		_renderPassRefs	.CreateOrThrow( _allocator.get() );
		GFX_DBG_ONLY( _allFeatureSets.CreateOrThrow( StdAlloc_t< FeatureSetName::Optimized_t >{ _allocator.get() });)
		...
		if ( NoBits( desc.options, EPipelinePackOpt::Pipelines ))
		{
			_file = null;
		}
...
*/
	Promise<RenderTechPipelinesPtr>  PPLNPACK::LoadRenderTechAsync (ResourceManager &resMngr, RenderTechName::Ref name, const RenderTechDesc &desc, PipelineCacheID cacheId) C_NE___
	{
		...
		return rt.LoadAsync( resMngr, desc, cacheId );
	}
...
*/
	RenderTechPipelinesPtr  PPLNPACK::LoadRenderTech (ResourceManager &resMngr, RenderTechName::Ref name, const RenderTechDesc &desc, PipelineCacheID cacheId) C_NE___
	{
		...
		auto&	rt = ConstCast(*_renTechs[ usize(it->second) ]);
		CHECK_ERR( rt.Load( resMngr, desc, cacheId ));
...
*/
	bool  PPLNPACK::_LoadFeatureSets (ResourceManager &resMngr, Bytes offset, Bytes size) __Th___
	{
		...
		if_likely( _parentPack != null )
		{
			_ForInternalUsage	parent_pack {*_parentPack};
...
*/
	bool  PPLNPACK::_CopyFeatureSets () __Th___
	{
		if ( _parentPack != null )
		{
			...
			*_unsupportedFS = parent_pack.GetUnsupportedFS();			// throw
			GFX_DBG_ONLY( *_allFeatureSets = parent_pack.GetAllFS();)	// throw
		}
		return true;
	}
...
*/
	...
	bool  PPLNPACK::_LoadPipelineArray (Serializing::Deserializer &des, OUT ArrayView<T> &arr) __NE___
	{
		...
		for (uint i = 0; result and (i < count); ++i)
		{
			result = PlacementNew<T>( OUT &ptr[i] )->Deserialize( des );
		}
		return result;
	}
...
*/
	bool  PPLNPACK::_LoadRenderTechniques (ResourceManager &resMngr, Serializing::Deserializer &des) __Th___
	{
		...
		for (uint i = 0; result and (i < count); ++i)
		{
			auto&	rt = *PlacementNew< StaticRC<RenderTech> >( OUT &_renTechs[i], *this );
...
*/
	bool  PPLNPACK::_LoadRTShaderBindingTable (Serializing::Deserializer &des) __NE___
	{
		...
		for (uint i = 0; result and (i < count); ++i)
		{
			result = PlacementNew< PipelineCompiler::SerializableRTShaderBindingTable >( OUT &ptr[i] )->Deserialize( des );
		}
		return result;
	}
...
*/
	bool  PPLNPACK::_LoadShaders (Serializing::Deserializer &des) __NE___
	{
		...
		for (uint i = 0; result and (i < count); ++i)
		{
			auto&	sh = *PlacementNew< ShaderModule >( OUT &_shaders[i] );
...
*/
	bool  PPLNPACK::RenderTech::Deserialize (ResourceManager &resMngr, Serializing::Deserializer &des) __Th___
	{
		...
		// passes
		{
			...
			{
				result = PlacementNew< SerializableRenderTechnique::Pass >( OUT &_passes[i] )->Deserialize( des );
			}
			CHECK_ERR( result );
		}
...
*/
	auto  PPLNPACK::RenderTech::LoadAsync (ResourceManager &resMngr, const RenderTechDesc &desc, PipelineCacheID cacheid) __NE___ -> Promise<RenderTechPipelinesPtr>
	{
		...
		_meshLocalSize		= desc.meshLocalSize;
		_computeLocalSize	= desc.computeLocalSize;
...
/*
=================================================
	_OverrideLocalGroupSize
=================================================
*/
	bool  PPLNPACK::RenderTech::_OverrideLocalGroupSize (INOUT WGLocalSize_t &localSize, const WGLocalSize_t &newLocalSize,
														 const WGLocalSizeSpec_t &localSizeSpec, const WGLocalSize_t &defaultLocalSize) __NE___
	{
		if ( Any( localSize == WGLocalSize_t{LoadTimeLocalSize} ))
		{
			CHECK_ERR( All( localSize	 == WGLocalSize_t{LoadTimeLocalSize} ));
			CHECK_ERR( All( newLocalSize != WGLocalSize_t{UndefinedLocalSize} ));

			bool3	mask		= (newLocalSize > WGLocalSize_t{1});
			bool3	has_spec	= (localSizeSpec != WGLocalSizeSpec_t{UndefinedSpecConst});
			bool3	default_one	= (defaultLocalSize == WGLocalSize_t{1});

			if ( not All( has_spec ))
			{
				CHECK_ERR( All( mask == has_spec ));
				CHECK_ERR( All( (not has_spec) == default_one ));
			}

			localSize = Select( has_spec, newLocalSize, WGLocalSize_t{UndefinedLocalSize} );
		}
		return true;
	}
...
*/
													...
													StringView dbgName) __NE___
	{
		...
		CHECK_ERR( _OverrideLocalGroupSize( INOUT spec.desc.taskLocalSize, _taskLocalSize, tmpl.taskLocalSizeSpec, tmpl.taskDefaultLocalSize ));
		CHECK_ERR( _OverrideLocalGroupSize( INOUT spec.desc.meshLocalSize, _meshLocalSize, tmpl.meshLocalSizeSpec, tmpl.meshDefaultLocalSize ));
...
*/
													   ...
													   StringView dbgName) __NE___
	{
		...
		CHECK_ERR( _OverrideLocalGroupSize( INOUT spec.desc.localSize, _computeLocalSize, tmpl.localSizeSpec, tmpl.defaultLocalSize ));
