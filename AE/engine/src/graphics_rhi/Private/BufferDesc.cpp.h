...
*/
	void BufferDesc::Validate () __NE___
	{
		...
		if ( NoBits( memType, EMemoryType::DeviceLocal ))
		{
			...
			usage   &= ~EBufferUsage_RequireDeviceLocal;
		}
		...
		if ( AnyBits( usage, EBufferUsage_RequireDevAddress ))
...
*/
	void BufferViewDesc::Validate (const BufferDesc &desc) __NE___
	{
		...
		ASSERT( AnyBits( desc.usage, EBufferUsage_AllowBufferView ));

		offset	= Min( offset, desc.size );
