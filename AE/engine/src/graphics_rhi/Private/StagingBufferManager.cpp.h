...
*/
	void  STBUFMNGR::GetImageRanges (OUT StagingImageResultRanges &result, const UploadImageDesc &uploadDesc, const ImageDesc &imgDesc,
									 const Bytes maxSize, const uint3 &imageGranularity, const FrameUID frameId, const Bool upload) __NE___
	{
		ASSERT( _frameId.load() == frameId );
		...
		const uint3		region_dim			= Min( mip_dim - img_offset, Max( uint3{uploadDesc.imageDim}, 1u ));
		...
		total_size = Min( total_size, maxSize );

		CHECK_ERRV( All( img_offset < mip_dim ));
		CHECK_ERRV( All( img_offset + region_dim <= mip_dim ));
		CHECK_ERRV( All( IsMultipleOf( uint2{img_offset}, texblock_dim )));
		...
		result.planeScaleY		= PowerOfTwo(0);
		...
		if ( AnyEqual( uploadDesc.heapType, EStagingHeapType::Static, EStagingHeapType::Any ))
		{
			...
			Unused( _AllocStaticImage( total_size, row_pitch, slice_pitch, mem_offset_align, texblock_dim,
										img_offset, region_dim, INOUT result, sb ));
			return;
		}
...
*/
									 ...
									 Bytes maxSize, const uint3 &imageGranularity, FrameUID frameId, Bool upload) __NE___
	{
		...
		CHECK_ERRV( All( img_offset < uint3{plane_dim,1} ));
		CHECK_ERRV( All( img_offset + uint3{region_dim,1} <= uint3{plane_dim,1} ));
		...
		if ( AnyEqual( uploadDesc.heapType, EStagingHeapType::Static, EStagingHeapType::Any ))
		{
			...
			Unused( _AllocStaticImage( total_size, row_pitch, slice_pitch, mem_offset_align, uint2{1},
										img_offset, uint3{region_dim,1}, INOUT result, sb ));
			return;
		}
	}
...
*/
	bool  STBUFMNGR::_InitDynamicBuffers (const GraphicsCreateInfo &info) __NE___
	{
		...
		_dynamic.blockSize			= Max( CeilPOT( info.staging.dynamicBlockSize ), 4_KiB );
