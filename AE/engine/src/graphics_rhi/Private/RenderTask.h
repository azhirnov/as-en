...
namespace AE::Graphics
{
	...
	// IAsyncTask
	public:
			void			OnCancel ()				__NE_OV	{ SkipCommandBuffer();  IAsyncTask::OnCancel(); }
	...
	protected:
		void  OnFailure ()							__NE___	{ SkipCommandBuffer();  IAsyncTask::OnFailure(); }
...
*/
	inline RenderTask::~RenderTask () __NE___
	{
		ASSERT_MSG( not IsValid(),
			"Call 'Execute(ctx)' to add command buffer to pending for submission queue or "
			"call 'OnFailure()' or 'SkipCommandBuffer()' to mark command buffer as unused, so other pending command buffers can be submitted when them ready." );
	}
...
=================================================
*/
	inline void  RenderTask::SkipCommandBuffer () __NE___
	{
		if_likely( IsValid() )
		{
...
namespace AE::Threading::_hidden_
{
	...
	public:
		...
		public:
			...
			void  SkipCommandBuffer ()											__NE___	{ return RenderTask::SkipCommandBuffer(); }
...
namespace AE::Graphics
{
	...
	//
	// Render Task Skip Command Buffer
	//
	struct RenderTask_SkipCommandBuffer
	{
		explicit RenderTask_SkipCommandBuffer ()	__NE___ {}
