...
=================================================
*/
	TRANSFER_CTX( template <typename ID> void, _UploadImage )(ID imageId, const UploadImageDesc &uploadDesc, const Bytes maxSize, OUT ImageMemView &memView) __Th___
	{
		StaticAssert( IsSame< ID, ImageID > or IsSame< ID, VideoImageID >);
		...
		const auto			handle		= [&dst_img]() {
												if constexpr( IsSame< ID, ImageID >)		return dst_img.Handle();
												if constexpr( IsSame< ID, VideoImageID >)	return dst_img.GetImageHandle();
											}();

		STAGINGBUF_MNGR::StagingImageResultRanges	res;
		sbm.GetImageRanges( OUT res, uploadDesc, img_desc, maxSize, MinImageTransferGranularity(), GetFrameId(), True{"upload"} );
...
*/
	...
	{
		StaticAssert( IsSame< StreamType, ImageStream > or
					  IsSame< StreamType, VideoImageStream >);
		...
		STAGINGBUF_MNGR&	sbm			= this->_mngr.GetStagingManager();
		const auto			handle		= [&dst_img]() {
												if constexpr( IsSame< StreamType, ImageStream >)		return dst_img.Handle();
												if constexpr( IsSame< StreamType, VideoImageStream >)	return dst_img.GetImageHandle();
											}();
		...
		STAGINGBUF_MNGR::StagingImageResultRanges	res;
		sbm.GetImageRanges( OUT res, upload_desc, img_desc, UMax, MinImageTransferGranularity(), GetFrameId(), True{"upload"} );
...
*/
	TRANSFER_CTX( template <typename ID> ITransferContext::ReadbackImageResult2, _ReadbackImage )(ID imageId, const ReadbackImageDesc &readDesc) __Th___
	{
		...
		const auto			handle		= [&src_img]() {
												if constexpr( IsSame< ID, ImageID >)		return src_img.Handle();
												if constexpr( IsSame< ID, VideoImageID >)	return src_img.GetImageHandle();
											}();

		STAGINGBUF_MNGR::StagingImageResultRanges	res;
		sbm.GetImageRanges( OUT res, readDesc, img_desc, UMax, MinImageTransferGranularity(), GetFrameId(), False{"readback"} );
...
*/
	TRANSFER_CTX( template <typename StreamType> ITransferContext::ReadbackImageResult, _ReadbackImage )(INOUT StreamType &stream) __Th___
	{
		...
		STAGINGBUF_MNGR&	sbm			= this->_mngr.GetStagingManager();
		const auto			handle		= [&src_img]() {
												if constexpr( IsSame< StreamType, ImageStream >)		return src_img.Handle();
												if constexpr( IsSame< StreamType, VideoImageStream >)	return src_img.GetImageHandle();
											}();
		...
		STAGINGBUF_MNGR::StagingImageResultRanges	res;
		sbm.GetImageRanges( OUT res, read_desc, img_desc, UMax, MinImageTransferGranularity(), GetFrameId(), False{"readback"} );
