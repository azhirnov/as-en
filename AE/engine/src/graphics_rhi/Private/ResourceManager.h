...
namespace AE::Graphics
{
	...
	private:
		...
		using SamplerPool_t			= StPoolTmpl< AE_PRIVATE_UNITE_RAW( SUFFIX, Sampler			),		SamplerID,					2u << 10 >;
		using RenderPassPool_t		= StPoolTmpl< AE_PRIVATE_UNITE_RAW( SUFFIX, RenderPass		),		RenderPassID,				128 >;
		...
		struct alignas(AE_CACHE_LINE) ExpiredResArray
		{
		  ...
		  #ifdef AE_ENABLE_VULKAN
			RingBuffer< uint >			framebuffers;
		  #endif
		};
		static constexpr uint		ExpiredResFrameOffset		= 2;
		static constexpr uint		DelayFramebufferDeletion	= GraphicsConfig::MaxSwapchainLength * 2;	// frames
	...
	public:
		...
		ND_ AsyncRTechPipelines			LoadRenderTechAsync (PipelinePackID packId, RenderTechName::Ref, const RenderTechDesc &  = Default, PipelineCacheID = Default)	__NE_OV;
		ND_ RenderTechPipelinesPtr		LoadRenderTech      (PipelinePackID packId, RenderTechName::Ref, const RenderTechDesc &  = Default, PipelineCacheID = Default)	__NE_OV;
	...
	// resource api
		...
		ND_ auto  _CreateResource (StringView msg, Args&& ...args)				__NE___ -> Strong<ID>;
	...
	private:
			void			_DelayedReleaseFramebuffer (VFramebufferID)					__NE___;
			void			_ReleaseFramebuffers (FrameUID, RingBuffer<uint> &)			__NE___;
			static void		_AddFrameIDtoFramebufferList (FrameUID, RingBuffer<uint> &)	__NE___;
...
*/
	...
	{
		if constexpr( IsSpecializationOf< Arg0, Strong >)
			StaticAssert( ExpResourceTypes_t::HasType< typename Arg0::ID_t >);
		else{
			StaticAssert( ExpResourceTypes_t::HasType< Arg0 >);
			StaticAssert( not IsHandleTmpl< Arg0 >);	// must be Strong<>
		}
