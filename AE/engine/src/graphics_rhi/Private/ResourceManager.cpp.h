...
# define ERR_MSG( _msg_, _dbgName_ )	(_msg_ ": "s << _dbgName_)
#else
# define ERR_MSG( _msg_, _dbgName_ )	_msg_
#endif
//-----------------------------------------------------------------------------
...
*/
	...
	forceinline Strong<ID>  ResourceManager::_CreateResource (StringView msg, Args&& ...args) __NE___
	{
		ID	id;
...
=================================================
*/
	RenderTechPipelinesPtr  ResourceManager::LoadRenderTech (PipelinePackID packId, RenderTechName::Ref name, const RenderTechDesc &desc, PipelineCacheID cache) __NE___
	{
		auto*	pack = GetResource( packId ? packId : _defaultPack.Get() );
		CHECK_ERR( pack != null );

		return pack->LoadRenderTech( *this, name, desc, cache );
	}
...
=================================================
*/
	Promise<RenderTechPipelinesPtr>  ResourceManager::LoadRenderTechAsync (PipelinePackID packId, RenderTechName::Ref name, const RenderTechDesc &desc, PipelineCacheID cache) __NE___
	{
		auto*	pack = GetResource( packId ? packId : _defaultPack.Get() );
		CHECK_ERR( pack != null );

		return pack->LoadRenderTechAsync( *this, name, desc, cache );
	}
...
*/
	...
	{
		return _CreateResource<PipelineLayoutID>(
					ERR_MSG( "failed when creating pipeline layout", ci.dbgName ),
					*this, ci );
	}
...
*/
	Strong<PipelinePackID>  ResourceManager::LoadPipelinePack (const PipelinePackDesc &desc) __NE___
	{
		...
		if_unlikely( not data.Create( *this, desc, id ))
		{
			data.Destroy( *this );
			_Unassign( id );
			RETURN_ERR( ERR_MSG( "failed when creating pipeline pack", desc.dbgName ));
		}

		data.AddRef();

		return Strong<PipelinePackID>{ id };
	}
...
# ifdef AE_ENABLE_VULKAN
	bool  ResourceManager::IsSupported (const RTClusterInfo &info) C_NE___
	{
		return VRTCluster::IsSupported( *this, info );
	}

	bool  ResourceManager::IsSupported (const RTPartitionedSceneInfo &info) C_NE___
	{
...
*/
	...
	{
		return _CreateResource<PipelineCacheID>(
					ERR_MSG( "failed when creating pipeline cache", dbgName ),
					*this, dbgName, RVRef(stream) );
	}
...
*/
	...
	{
		auto	id = _CreateResource<DescriptorSetLayoutID>( "failed when creating empty descriptor set layout"sv, GetDevice(), "EmptyDSLayout" );
		if ( id ) {
			_emptyDSLayout.Attach( id.Release() );
			return true;
		}
		return false;
	}
...
*/
	void  ResourceManager::_OnBeginFrame (FrameUID frameId, const BeginFrameConfig &cfg) __NE___
	{
		_expiredResources._currentFrameId.store( frameId );
		{
		  ...
		  #ifdef AE_ENABLE_VULKAN
			_AddFrameIDtoFramebufferList( frameId, INOUT list.framebuffers );
		  #endif
		}
...
*/
	ResourceManager::ExpiredResources::ExpiredResources () __Th___
	{
		for (auto& item : _list)
		{
			item.resources.reserve( 1u << 10 );		// throw

		  #ifdef AE_ENABLE_VULKAN
			item.framebuffers.reserve( 1u << 7 );	// throw
		  #endif
		}
	}
