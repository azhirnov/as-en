...
namespace AE::Graphics
{
	...
	// methods
	public:
		__Cx__ MetalBindingPerStage ()							__NE___	{}
		__Cx__ explicit MetalBindingPerStage (ubyte idx)		__NE___	: _data{ idx, idx, idx, UMax } {}
		...
		NdCx__ static int	ShaderToIndex (EShader type)		__NE___
		{
			switch ( type ) {
				case EShader::Vertex :
				case EShader::Tile :
		...
		Nd____ ubyte const*		PtrForShader (EShaderStages stage)	C_NE___	{ int i = StageToIndex( stage );  return i >= 0 ? &_data[i] : null; }
		Nd____ ubyte*			PtrForShader (EShaderStages stage)	__NE___	{ int i = StageToIndex( stage );  return i >= 0 ? &_data[i] : null; }

		Nd____ ubyte const*		PtrForShader (EShader type)			C_NE___	{ int i = ShaderToIndex( type );  return i >= 0 ? &_data[i] : null; }
		Nd____ ubyte*			PtrForShader (EShader type)			__NE___	{ int i = ShaderToIndex( type );  return i >= 0 ? &_data[i] : null; }

		Nd____ ubyte			ForShader (EShaderStages stage)		C_NE___	{ auto* ptr = PtrForShader( stage );  return ptr != null ? *ptr : UMax; }
		Nd____ ubyte			ForShader (EShader type)			C_NE___	{ auto* ptr = PtrForShader( type );   return ptr != null ? *ptr : UMax; }
	};
	...
	// methods
	public:
		__Cx__ DescSetBinding ()											__NE___	{}
		__Cx__ explicit DescSetBinding (uint vulkanBinding)					__NE___	: vkIndex{vulkanBinding} {}
		__Cx__ explicit DescSetBinding (MetalBindingPerStage metalBinding)	__NE___	: mtlIndex{metalBinding} {}

		NdCx__ bool  operator == (const DescSetBinding &rhs)				C_NE___	{ return vkIndex == rhs.vkIndex; }

		NdCx__ operator bool ()												C_NE___	{ return vkIndex != UMax; }
	};
	...
	public:
		...
		NdCx__ explicit operator bool ()									C_NE___	{ return stage != Default; }
	};
	...
	public:
		...
		ND_ virtual uint  RayTracingSceneCount (UniformName::Ref)																							C_NE___ = 0;
			...
			virtual bool  BindRayTracingPartitionedScene (UniformName::Ref, DeviceAddress scene, uint elementIndex = 0)										__NE___	= 0;
			virtual bool  BindRayTracingPartitionedScene (UniformName::Ref, BufferID buffer, Bytes offset = 0_b, uint elementIndex = 0)						__NE___	= 0;
		ND_ virtual uint  RayTracingPartitionedSceneCount (UniformName::Ref)																				C_NE___ = 0;
	};
