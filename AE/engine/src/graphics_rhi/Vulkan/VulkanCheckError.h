...
#	define VK_CHECK( /* expr */... )															\
	{																							\
		...
		AE::Base::Unused( AE::Graphics::__vk_CheckErrors( __vk_err__,							\
...
#	define AE_PRIVATE_VK_CHECK_R( _expr_, _msg_, _ret_ )										\
	{																							\
		const ::VkResult __vk_err__ = (_expr_);													\
		if_unlikely( not AE::Graphics::__vk_CheckErrors( __vk_err__,							\
...
# define VK_CHECK_ERR( /* expr, return_on_error */... )											\
		AE_PRIVATE_VK_CHECK2_R( AE_PRIVATE_GETARG_0( __VA_ARGS__, ),							\
