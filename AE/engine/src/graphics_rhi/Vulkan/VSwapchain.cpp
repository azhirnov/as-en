...
*/
	VSwapchain::VSwapchain () __NE___ :
		_indices{ MutableIdxBits{} }, _transform{ ESurfaceTransform::Identity }
	{}
...
*/
	VSwapchain::VSwapchainDesc  VSwapchain::GetVDescription () C_NE___
	{
		...
		result.preTransform		= VEnumCast( GetSurfaceTransform() );
		return result;
	}
...
*/
	VkResult  VSwapchain::AcquireNextImage () __NE___
	{
		...
		VkSemaphore	sem   = _imageAvailableSem[ cur_idx.semaphoreId ];
		uint		index = UMax;
		VkResult	res	  = _device->vkAcquireNextImageKHR( _device->GetVkDevice(), _vkSwapchain, UMax, sem, Default, OUT &index );
...
*/
	...
	{
		return Present( queue, Default, frameId );
	}

	VkResult  VSwapchain::Present (VQueuePtr queue, ArrayView<VkSemaphore> renderFinished, FrameUID frameId) __NE___
	{
		SHAREDLOCK( _guard );
		CHECK_ERR( _vkSwapchain != Default, VK_RESULT_MAX_ENUM );
		...
		frame_boundary.frameID	= ulong(frameId.Unique());
...
=================================================
*/
	bool  VSwapchain::ShouldRecreate () C_NE___
	{
		SHAREDLOCK( _guard );
		...
		uint2	size		= GetSurfaceSize();
		auto	transform	= GetSurfaceTransform();

		if ( transform != AEEnumCast( surf_caps.currentTransform ))
			return true;

		if ( Any( size != uint2{surf_caps.currentExtent.width, surf_caps.currentExtent.height} ))
			return true;
...
*/
										  ...
										  StringView				dbgName) __NE___
	{
		...
		const auto	transform		= AEEnumCast( swapchain_info.preTransform );
		const bool	is_90deg_rot	= SurfaceTransformUtils::IsAny90deg( transform );

		if ( is_90deg_rot )
			std::swap( swapchain_info.imageExtent.width, swapchain_info.imageExtent.height );
...
*/
													  ...
													  const VkSurfaceCapabilitiesKHR &surfaceCaps) C_NE___
	{
		...
		// On Wayland, Fuchsia and some other platforms:
		// "currentExtent is the special value (0xFFFFFFFF, 0xFFFFFFFF), indicating that the surface size
		...
		{
			float	ratio	= (float(extent.width) * float(surfaceCaps.currentExtent.width)) /
							  (float(extent.height) * float(surfaceCaps.currentExtent.height));

			// sometimes transformation is not applied
			if ( Abs( ratio - 1.0f ) < 0.01f )
			{
				swap = true;
				AE_LOG_DBG( "Surface rotation is not yet applied to extents." );
			}
...
*/
	void  VSwapchainInitializer::_GetSurfaceTransform (INOUT VkSurfaceTransformFlagBitsKHR &transform,
													   const VkSurfaceCapabilitiesKHR &surfaceCaps) C_NE___
	{
		if ( not IsSingleBitSet( transform ))
		{
			// use pre-transform
			// it is better for performance on Android
			// see https://github.com/KhronosGroup/Vulkan-Samples/tree/main/samples/performance/surface_rotation
		...
		// bad for performance on Android
...
*/
	void  VSwapchainInitializer::_PrintInfo (const VkSwapchainCreateInfoKHR &info) C_NE___
	{
		AE_LOGI( "Created Vulkan Swapchain:"s
			...
			<< "\n  dimension:       " << ToString( info.imageExtent.width ) << ", " << ToString( info.imageExtent.height )
			<< "\n----"
		);
	}
...
*/
	void  VSwapchainInitializer::_DestroySemaphores () __NE___
	{
		...
		// no way to know that semaphore is not used by presenting engine
