...
*/
	void  VDeviceInitializer::_OnCreateInstance (ArrayView<const char*> instanceExtensions, ArrayView<const char*> instanceLayers) __Th___
	{
		...
		_extensions.debugReport		= false;
		_extensions.debugUtils		= false;
		_extensions.validationFlags	= false;
	  #endif
	}
...
*/
	void  VDeviceInitializer::_SetResourceFlags (OUT ResourceFlags &outResFlags) C_NE___
	{
		...
		// descriptor types
		{
			...
			if ( props.partitionedAccelStructNVFeats.partitionedAccelerationStructure )
				outResFlags.descrTypes.insert( EDescriptorType::RayTracingPartitionedScene );

			StaticAssert( uint(EDescriptorType::_Count) == 13 );
		}
		...
		// image & buffer flags
		{
			...
			if ( props.accelerationStructureFeats.accelerationStructure )
				outResFlags.bufferUsage |= EBufferUsage::ASBuild_Scratch | EBufferUsage::ASBuild_ReadOnly | EBufferUsage::RTAS_Storage;
...
*/
	...
	bool  VDeviceInitializer::_CreateLogicalDevice (const DeviceCreateInfo &devCI) __Th___
	{
		...
		// setup extensions
		Array<const char *>		device_extensions = _GetDeviceExtensions( _vkDeviceVersion );					// throw	// in 'vk_features.h'
		...
		StaticArray< VkDeviceQueueGlobalPriorityCreateInfoKHR, max_queues >	global_priority = {};
		{
			...
			for (usize i = 0; i < queue_infos.size(); ++i)
			{
				...
				if ( _extensions.queueGlobalPriority )
				{
					...
					gprior.pNext			= null;
					gprior.globalPriority	= VK_QUEUE_GLOBAL_PRIORITY_MAX_ENUM;
			...
			for (usize i = 0, cnt = _queueCount; i < cnt; ++i)
			{
				...
				if ( _extensions.queueGlobalPriority )
				{
					if ( gprior.globalPriority == VK_QUEUE_GLOBAL_PRIORITY_MAX_ENUM )
		...
		void**	dev_info_last_pnext = null;
		{
			...
			if ( devCI.disableFeatures != null )
				devCI.disableFeatures( devCI.disableFeaturesUserData, INOUT _properties );
	  ...
	  #ifndef AE_CFG_RELEASE
		VkDeviceDeviceMemoryReportCreateInfoEXT		mem_report_ci = {};
		if ( _extensions.memoryReport and AllBits( devCI.devFlags, EDeviceFlags::EnableMemoryReport ))
		{
			mem_report_ci.sType				= VK_STRUCTURE_TYPE_DEVICE_DEVICE_MEMORY_REPORT_CREATE_INFO_EXT;
			mem_report_ci.pfnUserCallback	= &_DeviceMemoryReportCallback;
			mem_report_ci.pUserData			= this;

			*dev_info_last_pnext = &mem_report_ci;
			dev_info_last_pnext  = const_cast<void**>( &mem_report_ci.pNext );
		}
	  #endif

		dev_info_last_pnext = null;
...
/*
=================================================
	_SetupDeviceExtensions
=================================================
*/
	void  VDeviceInitializer::_SetupDeviceExtensions (INOUT VExtensions &ext) C_NE___
	{
...
*/
	void  VDeviceInitializer::_SetupFeatures (INOUT VProperties &feats) C_NE___
	{
	  ...
	  #ifdef AE_PLATFORM_WINDOWS
		if ( feats.rayTracingValidationFeats.rayTracingValidation )
		{
			if ( not WindowsUtils::HasEnvironmentVariable( "NV_ALLOW_RAYTRACING_VALIDATION" ))
				AE_LOGW( "Add environment variable 'NV_ALLOW_RAYTRACING_VALIDATION=1' to enable ray tracing validation" );
		}
	  #endif
	}
...
*/
	void  VDeviceInitializer::_LogLogicalDevice () C_Th___
	{
		...
		str	<< _GetVulkanExtensionsString();	// in 'vk_features.h'
		str << "\n----";
		AE_LOGI( str );
...
*/
	void  VDeviceInitializer::_ValidateInstanceVersion (VkInstance instance, ArrayView<const char*> layers, ArrayView<const char*> extensions, INOUT uint &version) C_Th___
	{
		...
		if ( old_ver == min_ver )
			return;	// 1.0 is always min version
		...
		// but actual version is min of instance and physical device versions.
		{
			...
			{
				Array< const char* >	instance_layers;
				Array< const char* >	instance_extensions = _GetInstanceExtensions( InstanceVersion{VK_VERSION_MAJOR(min_ver), VK_VERSION_MINOR(min_ver)} );	// in 'vk_features.h'
...
=================================================
*/
	bool  VDeviceInitializer::CreateMemoryReport (MemoryReport_t callback) __NE___
	{
	#ifndef AE_CFG_RELEASE
		DRC_EXLOCK( _drCheck );
		CHECK_ERR( GetVkInstance() != Default );
		...
		{
			auto	dst = _memoryReport.WriteLock();

			dst->callback = RVRef(callback);
			return true;
		}
	#else
		Unused( callback );
...
*/
	VKAPI_ATTR void VKAPI_CALL
		VDeviceInitializer::_DeviceMemoryReportCallback (const VkDeviceMemoryReportCallbackDataEXT*  pCallbackData,
														 void*                                       pUserData) __NE___
	{
		NonNull( pCallbackData );
		ASSERT( pCallbackData->sType == VK_STRUCTURE_TYPE_DEVICE_MEMORY_REPORT_CALLBACK_DATA_EXT );

		auto*	self	= static_cast<VDeviceInitializer *>(pUserData);
		auto	mem_rep	= self->_memoryReport.ReadLock();

		if ( mem_rep->callback )
		{
			mem_rep->callback( AnyTypeCRef{*pCallbackData} );
		}
		else
		{
		#ifdef AE_ENABLE_LOGS
			String	str;
			str << "Memory report:"
				<< "\n  type:           " << VkDeviceMemoryReportEventTypeEXTToString( pCallbackData->type )
				<< "\n  memoryObjectId: " << ToString<16>( pCallbackData->memoryObjectId )
				<< "\n  size:           " << ToString( Bytes{pCallbackData->size} )
				<< "\n  objectType:     " << VkObjectTypeToString( pCallbackData->objectType )
				<< "\n  objectHandle:   " << ToString<16>( pCallbackData->objectHandle );

			if ( AnyEqual( pCallbackData->type, VK_DEVICE_MEMORY_REPORT_EVENT_TYPE_ALLOCATE_EXT, VK_DEVICE_MEMORY_REPORT_EVENT_TYPE_ALLOCATION_FAILED_EXT ))
				str << "\n  heapIndex:      " << ToString( pCallbackData->heapIndex );
...
*/
	bool  VDeviceInitializer::Init (const VDeviceInitializer &otherDev) __NE___
	{
		...
		_CheckInstanceExtensions();	// in 'vk_features.h'
		_CheckDeviceExtensions();	// in 'vk_features.h'
