...
*/
	...
	bool  DescriptorUpdater::_BindImages (UniformName::Ref name, ArrayView<T> images, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T, ImageViewID >)
		{
		...
		if constexpr( IsSame< T, VkImageView >)
		{
			for (usize i = 0; i < images.size(); ++i)
			{
...
*/
	...
	bool  DescriptorUpdater::_BindTextures (UniformName::Ref name, ArrayView<T1> images, const T2 &sampler, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T1, ImageViewID > and IsSame< T2, SamplerName >)
		{
		...
		if constexpr( IsSame< T1, VkImageView > and IsSame< T2, VkSampler >)
		{
...
*/
	...
	bool  DescriptorUpdater::_BindSamplers (UniformName::Ref name, ArrayView<T> samplers, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T, SamplerName >)
		{
		...
		if constexpr( IsSame< T, VkSampler >)
		{
			for (usize i = 0; i < samplers.size(); ++i)
			{
...
*/
	...
	bool  DescriptorUpdater::_BindBuffers (UniformName::Ref name, ShaderStructName::Ref typeName, ArrayView<T> buffers, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T, BufferID >)
		{
		...
		if constexpr( IsSame< T, VkBuffer >)
		{
			for (usize i = 0; i < buffers.size(); ++i)
			{
...
*/
	...
	bool  DescriptorUpdater::_BindTexelBuffers (UniformName::Ref name, ArrayView<T> views, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T, BufferViewID >)
		{
		...
		if constexpr( IsSame< T, VkBufferView >)
		{
			for (usize i = 0; i < views.size(); ++i)
			{
...
*/
	...
	bool  DescriptorUpdater::_BindRayTracingScenes (UniformName::Ref name, ArrayView<T> scenes, const uint firstIndex) __NE___
	{
		...
		if constexpr( IsSame< T, RTSceneID >)
		{
		...
		if constexpr( IsSame< T, VkAccelerationStructureKHR >)
		{
			for (usize i = 0; i < scenes.size(); ++i)
			{
...
*/
	...
	bool  DescriptorUpdater::_BindRayTracingPartitionedScenes (UniformName::Ref name, ArrayView<T> scenes, const uint firstIndex) __NE___
	{
		...
		auto [un, off] = _FindUniform<DT::RayTracingPartitionedScene>( name );

		CHECK_ERR( un != null );
		CHECK_ERR( un->type == DT::RayTracingPartitionedScene );
		CHECK_ERR( firstIndex + uint(scenes.size()) <= un->arraySize );
		...
		if ( _UseUpdateTemplate() )
		{
			dst = Cast<VkDeviceAddress>( _updDescTempl.tmplData + *off + SizeOf<VkDeviceAddress> * firstIndex );
		}
		else
		{
			if_unlikely( _updDesc.index+1 >= _updDesc.count )
			...
			dst = _allocator.Allocate< VkDeviceAddress >( scenes.size() );

			auto&	as_wds = *_allocator.Allocate< VkWriteDescriptorSetPartitionedAccelerationStructureNV >(1);
			as_wds.sType						= VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET_PARTITIONED_ACCELERATION_STRUCTURE_NV;
			as_wds.pNext						= null;
			as_wds.accelerationStructureCount	= uint(scenes.size());
			as_wds.pAccelerationStructures		= dst;
			...
			wds.sType			= VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			wds.pNext			= &as_wds;
			wds.descriptorType	= VK_DESCRIPTOR_TYPE_PARTITIONED_ACCELERATION_STRUCTURE_NV;
			wds.dstArrayElement	= firstIndex;
			wds.descriptorCount	= uint(scenes.size());
			wds.dstBinding		= un->binding.vkIndex;
			wds.dstSet			= _dsHandle;
		...
		if constexpr( IsSame< T, Pair<BufferID, Bytes> >)
		{
			for (usize i = 0; i < scenes.size(); ++i)
			{
				auto*	buf = _resMngr.GetResource( scenes[i].first );
				CHECK_ERR( buf != null );

				GFX_DBG_ONLY(
					const auto	desc = buf->Description();
					CHECK( AllBits( desc.usage, EBufferUsage::RTAS_Storage ));
					CHECK( scenes[i].second < desc.size );
				)
				dst[i] = BitCast<VkDeviceAddress>( buf->GetDeviceAddress() + scenes[i].second );
			}
			return true;
		}
		...
		if constexpr( IsSame< T, DeviceAddress >)
		{
			for (usize i = 0; i < scenes.size(); ++i)
			{
				...
				dst[i] = BitCast<VkDeviceAddress>( scenes[i] );
			}
			return true;
		}

		if constexpr( IsSame< T, VkDeviceAddress >)
		{
			for (usize i = 0; i < scenes.size(); ++i)
			{
				CHECK_ERR( scenes[i] != Default );
