...
=================================================
	SetRobustness
----
	BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS
		Specifies that accesses to buffers are bounds-checked against the range of the buffer descriptor.
		Out of bounds accesses must not cause application termination, and the effects of shader loads, stores,
		and atomics must conform to an implementation-dependent behavior...

	BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS_2
		Indicates whether buffer accesses are tightly bounds-checked against the range of the descriptor.
		Out of bounds buffer loads will return zero values, and image load, sample, and atomic operations
		from texel buffers will have (0,0,1) values inserted for missing G, B, or A components based on the format.

    IMAGE_BEHAVIOR_ROBUST_IMAGE_ACCESS
		Indicates whether image accesses are tightly bounds-checked against the dimensions of the image view.
		Invalid texels resulting from out of bounds image loads will be replaced as described in Texel Replacement,
		with either (0,0,1) or (0,0,0) values inserted for missing G, B, or A components based on the format.

    IMAGE_BEHAVIOR_ROBUST_IMAGE_ACCESS_2
		Indicates whether image accesses are tightly bounds-checked against the dimensions of the image view.
		Out of bounds image load, sample, and atomic operations from images will return zero values, with (0,0,1)
		values inserted for missing G, B, or A components based on the format.
=================================================
*/
	void  SetRobustness (OUT VkPipelineRobustnessCreateInfoEXT &ci) __NE___
	{
		ci.sType	= VK_STRUCTURE_TYPE_PIPELINE_ROBUSTNESS_CREATE_INFO_EXT;
		ci.pNext	= null;

	#if 0
		// crash on out of bounds access
		ci.storageBuffers	= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DISABLED_EXT;
		ci.uniformBuffers	= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DISABLED_EXT;
		ci.vertexInputs		= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DISABLED_EXT;
		ci.images			= VK_PIPELINE_ROBUSTNESS_IMAGE_BEHAVIOR_DISABLED_EXT;
	#else
		ci.storageBuffers	= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT_EXT;
		ci.uniformBuffers	= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT_EXT;
		ci.vertexInputs		= VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT_EXT;
		ci.images			= VK_PIPELINE_ROBUSTNESS_IMAGE_BEHAVIOR_DEVICE_DEFAULT_EXT;
	#endif
	}
