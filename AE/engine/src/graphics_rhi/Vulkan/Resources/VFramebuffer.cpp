...
*/
	bool  VFramebuffer::Create (ResourceManager &resMngr, const RenderPassDesc &rpDesc, RenderPassID rpId, StringView dbgName) __NE___
	{
		...
		_dimension	= ImageDim2_t{rpDesc.area.Size()};	// TODO: check
		...
		for (auto [name, att] : rpDesc.attachments)
		{
			...
			#if AE_DBG_GRAPHICS
			{
				const auto	img_desc	= resMngr.GetDescription( view->ImageId() );
				const auto	rp_fmt		= ren_pass->GetPixelFormat( idx );

				if ( img_desc.format != rp_fmt )
				{
					String	str {"Attachment '"};
					str << resMngr.HashToName( name ) << "' has different pixel format than in render pass '"
						<< ren_pass->GetDebugName() << "'\n" << ToString( img_desc.format ) << " != " << ToString( rp_fmt );
					RETURN_ERR( str );
				}
