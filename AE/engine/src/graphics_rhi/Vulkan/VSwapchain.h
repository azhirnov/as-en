...
namespace AE::Graphics
{
	...
	protected:
		...
		static constexpr uint	_SemaphoreIndexBits	= CT_IntLog2< _MaxSwapchainLength >;
		...
		{
			uint	semaphoreId		: _SemaphoreIndexBits;
			uint	imageIdx		: _ImageIndexBits;

			MutableIdxBits () __NE___ : semaphoreId{0}, imageIdx{_MaxImageIndex} {}
		};

		StaticAssert( (1u << _SemaphoreIndexBits) == _MaxSwapchainLength );
	...
	public:
		...
		struct VSwapchainDesc
		{
			...
			VkSurfaceTransformFlagBitsKHR	preTransform		= VK_SURFACE_TRANSFORM_FLAG_BITS_MAX_ENUM_KHR;	// use surface transform
	...
	protected:
		...
		Atomic< ESurfaceTransform >		_transform;
	...
	public:
		...
		ND_ bool	ShouldRecreate ()											C_NE___;
		...
		ND_ VkResult  Present (VQueuePtr queue,
							   ArrayView<VkSemaphore> renderFinished,
							   FrameUID frameId = Default)						__NE___;
		ND_ VkResult  Present (VQueuePtr queue,
							   FrameUID frameId = Default)						__NE___;
		...
		ND_ ESurfaceTransform	GetSurfaceTransform ()							C_NE___	{ return _transform.load(); }
