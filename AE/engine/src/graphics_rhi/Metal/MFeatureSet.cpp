...
*/
	void  MFeatureSet::InitFromFSTable (const MGPUFamilies &f, Version2 msl, INOUT Features &outFeats, INOUT Properties &outProps) __NE___
	{
		...
		outProps.maxBufferSize						= 64_KiB;	// [dev maxBufferLength]
		...
		outProps.maxLengthOfConstantBufferArgumentsPerFunction = 4_KiB;
...
*/
	void  MFeatureSet::InitFeatureSet (const MGPUFamilies &f, INOUT FeatureSet &outFeatureSet) C_NE___
	{
		...
		{
			// save input
			auto	devices = outFeatureSet.devicesIds;
			auto	vendors = outFeatureSet.vendorIds;
			auto	sh_ver	= outFeatureSet.maxShaderVersion;

			outFeatureSet.Init( False );

			// restore
			outFeatureSet.maxShaderVersion	= sh_ver;
			outFeatureSet.devicesIds		= devices;
			outFeatureSet.vendorIds			= vendors;
		}
		...
		if ( mac20_ios22 )
		{
			...
			outFeatureSet.minSubgroupSize = outFeatureSet.maxSubgroupSize = POTValue_From<4>;	// require 'quadScopedPermuteOperations'
			...
			if ( features.SIMD_scopedPermuteOperations )
			{
				...
				// default
				outFeatureSet.minSubgroupSize = POTValue_From<4>;
				outFeatureSet.maxSubgroupSize = POTValue_From<32>;
					...
					case EGPUVendor::AMD :
					{
						...
						switch ( device_bits.ExtractFirst() )
						{
							...
							case EGraphicsDeviceID::AMD_RDNA2_APU :
								outFeatureSet.minSubgroupSize = POTValue_From<4>;
								outFeatureSet.maxSubgroupSize = POTValue_From<64>;
								break;
						}
						break;
					}
					case EGPUVendor::Intel :
						outFeatureSet.minSubgroupSize = POTValue_From<8>;
						outFeatureSet.maxSubgroupSize = POTValue_From<32>;
						break;
				}
			}
		...
		// shader features/limits
		{
			...
			outFeatureSet.maxDrawIndirectCount		= 1u << 30;		// TODO: check
			...
			outFeatureSet.perPipeline.maxInputAttachments	= properties.maxNumberOfTexturesInsideArgumentBuffer;
			outFeatureSet.perPipeline.maxSampledImages		= properties.maxNumberOfTexturesInsideArgumentBuffer;
			outFeatureSet.perPipeline.maxSamplers			= properties.maxNumberOfSamplersInsideArgumentBuffer;
			outFeatureSet.perPipeline.maxStorageBuffers		= properties.maxNumberOfBufferInsideArgumentBuffer;
			...
			outFeatureSet.maxComputeWorkGroupInvocations	= POTValue{ properties.maxThreadsPerThreadgroup };
			outFeatureSet.maxComputeWorkGroupSizeX			= POTValue{ Max( properties.maxThreadsPerThreadgroup3.x, properties.maxThreadsPerThreadgroup )};
			outFeatureSet.maxComputeWorkGroupSizeY			= POTValue{ Max( properties.maxThreadsPerThreadgroup3.y, properties.maxThreadsPerThreadgroup )};
			outFeatureSet.maxComputeWorkGroupSizeZ			= POTValue{ Max( properties.maxThreadsPerThreadgroup3.z, properties.maxThreadsPerThreadgroup )};
			...
			if ( features.meshShader )
			{
				...
				outFeatureSet.maxTaskWorkGroupSize					= POTValue{ properties.maxThreadsPerThreadgroup };
				outFeatureSet.maxMeshWorkGroupSize					= POTValue{ properties.maxThreadsPerThreadgroup };
				//		set default		// TODO: validate
				outFeatureSet.maxMeshOutputVertices					= POTValue_From< 128 >;
				outFeatureSet.maxMeshOutputPrimitives				= POTValue_From< 128 >;
				outFeatureSet.maxMeshOutputPerVertexGranularity		= POTValue_From< 32 >;
				outFeatureSet.maxMeshOutputPerPrimitiveGranularity	= POTValue_From< 32 >;
				outFeatureSet.maxTaskPayloadSize					= KiBytes{ 16<<10 };
				outFeatureSet.maxTaskSharedMemorySize				= KiBytes{ 16<<10 };
				outFeatureSet.maxTaskPayloadAndSharedMemorySize		= KiBytes{ 16<<10 };
				outFeatureSet.maxMeshSharedMemorySize				= KiBytes{ 16<<10 };
				outFeatureSet.maxMeshPayloadAndSharedMemorySize		= KiBytes{ 16<<10 };
				outFeatureSet.maxMeshOutputMemorySize				= KiBytes{ 16<<10 };
				outFeatureSet.maxMeshPayloadAndOutputMemorySize		= KiBytes{ 16<<10 };
				outFeatureSet.maxMeshMultiviewViewCount				= 0;
				outFeatureSet.maxPreferredTaskWorkGroupInvocations	= POTValue_From< 32 >;
				outFeatureSet.maxPreferredMeshWorkGroupInvocations	= POTValue_From< 32 >;
			}
		...
		// image
		{
			...
			outFeatureSet.maxImageDimension3D			= POTValue{ properties.max3DTextureSize };
			outFeatureSet.maxImageDimensionCube			= POTValue{ properties.maxCubemapTextureSize };
			outFeatureSet.maxImageArrayLayers			= POTValue{ properties.maxNumberOfLayersPerTextureArray_1D_2D_3D };
		...
		// framebuffer
		{
			...
			outFeatureSet.maxFramebufferLayers			= POTValue{ properties.maxNumberOfLayersPerTextureArray_1D_2D_3D };
		}
		// render pass
		{
