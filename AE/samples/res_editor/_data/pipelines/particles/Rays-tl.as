...
//-----------------------------------------------------------------------------
	...
	void Main ()
	{
		Particle		p			= un_Particles.elements[ gl.VertexIndex / 3 ];
		const float		size		= (p.position_size.w * 2.0 * iSize) / Max( un_PerPass.resolution.x, un_PerPass.resolution.y );
		const float3	vel			= Normalize(p.velocity_color.xyz) * Min( Length(p.velocity_color.xyz), size * 25.0 );
		...
		const uint		idx			= gl.VertexIndex % 3;
