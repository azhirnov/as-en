...
//-----------------------------------------------------------------------------
	...
	void Main ()
	{
		const float3	line_pos	= un_DrawTasks.tasks[ gl.InstanceIndex ].xyz;
		const float		scale		= un_DrawTasks.tasks[ gl.InstanceIndex ].w * 2.0;

		const uint		idx			= gl.VertexIndex;
		const float3	vert_pos	= line_pos + (un_Geometry.positions[idx] * scale);
		const float4	world_pos	= LocalPosToWorldSpace( vert_pos );
		float4			color		= float4(1.0, 0.2, 0.3, 1.0);

	#if 1
		if ( Frustum_TestLine(	un_PerPass.camera.frustum,
								line_pos - un_PerPass.camera.pos + float3(-1.0) * scale * iError,
								line_pos - un_PerPass.camera.pos + float3( 1.0) * scale * iError ))
	#else
		if ( Frustum_TestPoint(	un_PerPass.camera.frustum,
								line_pos - un_PerPass.camera.pos + float3(-1.0) * scale * iError ) and
			 Frustum_TestPoint(	un_PerPass.camera.frustum,
								line_pos - un_PerPass.camera.pos + float3(1.0) * scale * iError ))
	#endif
		{
			if ( iInvCulling == 0 )
