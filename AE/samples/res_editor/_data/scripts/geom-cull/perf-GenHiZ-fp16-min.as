...
/*
	...
	2. Calculate mipmaps.
		- used Min filter to avoid fp32 to fp16 rounding error.
		- Min filter used for default depth buffer (zero to one).

	Performance:
	* Used fp16 to store mipmaps.
*/
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		{
			d = Min( d, gl.texture.Fetch( un_HighMip, int2(x,y), 0 ).r);	// R32F
		}

		// correction is not needed (on NV ?)

		#ifdef SH_FRAG
			out_Color = float4(d);	// R16F
		#else
			gl.image.Store( un_OutImage, GetGlobalCoord().xy, float4(d) );	// R16F
		#endif
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		#else
		{
			...
			c = Min( c, gl.texture.Fetch( un_InImage, p + int2(1,0), 0 ).r );
			c = Min( c, gl.texture.Fetch( un_InImage, p + int2(0,1), 0 ).r );
			c = Min( c, gl.texture.Fetch( un_InImage, p + int2(1,1), 0 ).r );
		}
		#endif
