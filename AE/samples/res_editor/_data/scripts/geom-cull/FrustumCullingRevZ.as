...
//-----------------------------------------------------------------------------
	...
	void  DrawSpheres ()
	{
		...
		// render loop
		{
			...
			draw.AddPipeline( "*/../FrustumCulling/Boxes.ppln" );
	...
	void  DrawLines ()
	{
		...
		{
			RC<SceneGraphicsPass>	draw = scene.AddGraphicsPass( "main pass" );
			draw.AddPipeline( "*/../FrustumCulling/Lines.ppln" );
			draw.Output( "out_Color",	rt, RGBA32f(0.0) );
			draw.Output(				ds, DepthStencil( 0.f, 0 ));
			draw.Constant( "iRndColor",		dyn_rnd_color );
			draw.Constant( "iScale",		dyn_scale );
	...
	void  DrawCones ()
	{
		...
		{
			RC<SceneGraphicsPass>	draw = scene.AddGraphicsPass( "main pass" );
			draw.AddPipeline( "*/../FrustumCulling/Cones.ppln" );
			draw.Output( "out_Color",	rt, RGBA32f(0.0) );
			draw.Output(				ds, DepthStencil( 0.f, 0 ));
			draw.Constant( "iRndColor",		dyn_rnd_color );
			draw.Constant( "iScale",		dyn_scale );
	...
	void  DrawFrustum ()
	{
		...
		// render loop
		{
			...
			draw.AddPipeline( "*/../FrustumCulling/Dbg.ppln" );
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		Frustum	fr = Frustum_FromMatrix( un_PerPass.camera.viewProj, iClipPlanes.yx );
