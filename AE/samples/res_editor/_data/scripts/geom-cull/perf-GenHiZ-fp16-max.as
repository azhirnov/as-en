...
/*
	...
	2. Calculate mipmaps.
		- used Max filter to trigger fp32 to fp16 rounding error.
		- Max filter required for reverse-Z depth buffer (one to zero).

	Performance:
	* Used fp16 to store mipmaps which is 2x faster for memory access.
*/
...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		EPixelFormat		fmt			= EPixelFormat::R16F;
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		half	h0	= half(d);
		half	h1	= FastAddULP( h0 );
		float	diff = float(h1) - float(h0);
				diff *= iBiasScale + bias;

		out_Color.r = Saturate( float(h0) + diff );
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		// correction is needed only for Max filter, Min filter already use rounding down
		half	h = half(d);
		...
		#ifdef SH_FRAG
			out_Color = float4(h);	// R16F
		#else
			gl.image.Store( un_OutImage, GetGlobalCoord().xy, float4(h) );	// R16F
		#endif
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		#ifdef USE_REDUCTION
		{
			...
			c = gl.texture.SampleLod( un_InImage, uv, 0.0 ).r;	// R16F
		}
		#else
		{
			int2	p = GetGlobalCoord().xy * 2;
			c = gl.texture.Fetch( un_InImage, p, 0 ).r;		// R16F
			...
			out_Color.r = c;	// R16F
		#else
			gl.image.Store( un_OutImage, GetGlobalCoord().xy, float4(c) );	// R16F
		#endif
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		float	d0		= gl.texture.SampleLod( un_Mipmaps, uv, iMip ).r;	// R16F
