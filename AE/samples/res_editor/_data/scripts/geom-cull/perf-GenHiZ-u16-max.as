...
/*
	...
	2. Calculate mipmaps.
		- used Max filter to trigger fp32 to u16 rounding error.
		- Max filter required for reverse-Z depth buffer (one to zero).

	Performance:
	* Used unorm16 to store mipmaps which is 2x faster for memory access.
*/
...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		RC<DynamicUInt2>	tex_dim2	= tex_dim.Div(uint2(2)).NearPOT();
		RC<DynamicDim>		dim2		= tex_dim2.Dimension();
		RC<DynamicUInt>		correction	= DynamicUInt();

		EPixelFormat		fmt			= EPixelFormat::R16_UNorm;
		RC<Image>			high_mip	= Image( EPixelFormat::R32F, dim );					high_mip.Name( "High mip" );
		...
		Slider( correction,	"Correction",	0,	1,	1 );
...

		{
			...
			pass.Slider( "iBiasScale",	-1.0,	1.0,	0.5 );
		}
		...
		if ( not gfx_only )
		{
			...
			pass.Constant( "iCorrection",	correction );
			pass.LocalSize( 8, 8 );
		...
		if ( GetFeatureSet().hasSamplerFilterMinmax() )
		{
			...
			if ( not gfx_only )
			{
				...
				pass.Variable( "un_InImage",	"un_OutImage",	mipmaps,	Sampler_MaxLinearClamp );
				pass.Repeat( repeat );
				pass.MeasureTime( time2 );
				pass.EnableIfEqual( mode, 2 );
			}{
				RC<RasterMip>		pass = RasterMip( "", "GEN_MIPMAP; USE_REDUCTION" );
				pass.Variable( "un_InImage",	"out_Color",	mipmaps,	Sampler_MaxLinearClamp );
				pass.Repeat( repeat );
				pass.MeasureTime( time2 );
...
//-----------------------------------------------------------------------------
	...
	float  ToUNorm16 (float x)
	{
		uint	packed = packUnorm2x16( float2(x,x) );
		return unpackUnorm2x16( packed ).x;
	}

	void  Main ()
	{
		// generate 2x2 quads with same value
		float2	uv = float2(GetGlobalCoord().xy >> 1) * GetGlobalSizeRcp().xy + iHash;
		float	d  = Saturate( -0.1 +
								DHash12( uv * 141.2 ) * 0.4 +
								DHash12( Floor(uv * 40.0) * 28.33 ) * 0.4 +
								DHash12( Floor(uv * 10.0) * 55.77 ) * 0.4 +
								DHash12( Floor(uv * 1.0) * 55.77 ) * 0.4 );

		int2	quad = GetGlobalCoord().xy & 1;
		float	bias = (float(quad.x + quad.y*2) + 0.5) / 4.0;
				bias = ToSNorm( bias ) * 0.1;

		float	u0	= ToUNorm16( d );

		float	diff = 1.0 / 65535.0;
				diff *= iBiasScale + bias;

		out_Color.r = Saturate( u0 + diff );
	}
...
//-----------------------------------------------------------------------------
	...
	float  ConservativeR16UNormMax (float x)
	{
	//	return Saturate( x + (1.0 / 65535.0) );
		return Saturate( x + (0.5 / 65535.0) + 1.0e-8 );
	}

	// example: downsample 5x5 to 4x4
	//
	// |0|1|2|3|4|
	//  |0|1|2|3|
	...
	void  Main ()
	{
		...
		{
			d = Max( d, gl.texture.Fetch( un_HighMip, int2(x,y), 0 ).r);	// R32F
		}

		// correction for fp32 -> unorm16
		if ( iCorrection != 0 )
		{
			d = ConservativeR16UNormMax( d );
		}

		#ifdef SH_FRAG
			out_Color = float4(d);	// R16_UNorm
		#else
			gl.image.Store( un_OutImage, GetGlobalCoord().xy, float4(d) );	// R16_UNorm
		#endif
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		float	c;	// actually 'half', but performance is same

		#ifdef USE_REDUCTION
		{
			float2	uv = (float2(GetGlobalCoord().xy) + 0.5) * iInvResolution;
			c = gl.texture.SampleLod( un_InImage, uv, 0.0 ).r;	// R16_UNorm
		}
		#else
		{
			int2	p = GetGlobalCoord().xy * 2;
			c = gl.texture.Fetch( un_InImage, p, 0 ).r;		// R16_UNorm
			c = Max( c, gl.texture.Fetch( un_InImage, p + int2(1,0), 0 ).r );
			c = Max( c, gl.texture.Fetch( un_InImage, p + int2(0,1), 0 ).r );
			c = Max( c, gl.texture.Fetch( un_InImage, p + int2(1,1), 0 ).r );
		}
		#endif

		#ifdef SH_FRAG
			out_Color.r = c;	// R16_UNorm
		#else
			gl.image.Store( un_OutImage, GetGlobalCoord().xy, float4(c) );	// R16_UNorm
		#endif
	}
...
//-----------------------------------------------------------------------------
	...
	void  Main ()
	{
		...
		float	d1		= gl.texture.SampleLod( un_HighMip, uv, 0.0 ).r;	// R32F
		...
		float	d0		= gl.texture.SampleLod( un_Mipmaps, uv, iMip ).r;	// R16_UNorm
