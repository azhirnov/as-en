...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		// red graph
		{
			...
			pass.Slider( "iParams",	float3(0.0), float3(2.0),	float3(0.5, 1.0, 1.0) );
		...
		// green graph
		{
			...
			pass.OutputBlend( "out_Color",	rt,	EBlendFactor::SrcAlpha, EBlendFactor::One, EBlendOp::Add );
			pass.Slider( "iParams",	float3(0.0), float3(2.0),	float3(1.0, 1.0, 0.0) );
		...
		// blue graph
		{
			...
			pass.OutputBlend( "out_Color",	rt,	EBlendFactor::SrcAlpha, EBlendFactor::One, EBlendOp::Add );
			pass.Slider( "iParams",	float3(1.0, 1.0, -0.2), float3(10.0, 4.0, 1.0),	float3(2.0, 1.0, 0.0) );
...
//-----------------------------------------------------------------------------
	...
	{
		x = 1.0 - x;
		x = iParams.z / (-x * iParams.x + 1.0) - iParams.y;
		return 1.0 - x;
	}
	...
	{
		x = 1.0 - x;
		x = iParams.z / (-(x*x) * iParams.x + 1.0) - iParams.y;
		return 1.0 - x;
	}
...
//-----------------------------------------------------------------------------
	...
	float  Graph (float x)
	{
		float	a = Pow( 1.0/iParams.y, 1.0/iParams.x ) - 1.0;
		
		x = 1.0 - x;
		x = Pow( x + iParams.z + a, iParams.x ) * iParams.y;
		return 1.0 - x;
	}
...
//-----------------------------------------------------------------------------
	...
	{
		x = 1.0 - x;
		x = SquareRootEaseIn( x * iParams.x ) * iParams.y - iParams.z;
		return 1.0 - x;
	}
	...
	{
		x = 1.0 - x;
		x = CubicRootEaseIn( x * iParams.x ) * iParams.y - iParams.z;
		return 1.0 - x;
	}
