...
//-----------------------------------------------------------------------------
    ...
    void ASmain ()
    {
        ...
        // render loop
        {
            ...
            pass.RayMiss( MissIndex(0), RTShader("", "PRIMARY_MISS") );
            pass.RayMiss( MissIndex(1), RTShader("", "SHADOW_MISS") );
...
//-----------------------------------------------------------------------------
    ...
    void Main ()
    {
        ...
        HWTraceRay( un_RtScene, hwray, /*payload*/PRIMARY_RAY_INDEX );
...
//-----------------------------------------------------------------------------
    ...
    layout(location=SHADOW_RAY_INDEX)   gl::RayPayloadIn ShadowRayPayload  ShadowRay;
...
//-----------------------------------------------------------------------------
    ...
    layout(location=PRIMARY_RAY_INDEX)  gl::RayPayloadIn PrimaryRayPayload  PrimaryRay;
    layout(location=SHADOW_RAY_INDEX)   gl::RayPayload   ShadowRayPayload   ShadowRay;
    ...
    ND_ float  CastShadow (const float3 origin, const float3 dir, const float tmax)
    {
        ...
        hwray.sbtRecordOffset   = SHADOW_RAY_INDEX;
        hwray.sbtRecordStride   = HitGroupStride;
        hwray.missIndex         = SHADOW_RAY_INDEX;
        ...
        HWTraceRay( un_RtScene, hwray, /*payload*/SHADOW_RAY_INDEX );
    ...
    void Main ()
    {
        ...
        const float     shading         = CastShadow( world_pos, light_dir, dist_to_light );
