...
//-------------------------------------------------------------------------------------------------
	...
	//	float2	off	= 1.0 / float2(gl.texture.GetSize( un_Texture, 0 ));
	//	float2	off	= 1.0 / un_PerPass.resolution.xy * 1.0/2.0;
		float2	off = un_PerPass.invResolution * 0.5;

		float4	c0	= gl.texture.Sample( un_Texture, uv ) * 2.0;

		float4	c1	= gl.texture.Sample( un_Texture, uv + float2(-off.x, 0.0) );
		float4	c2	= gl.texture.Sample( un_Texture, uv + float2( off.x, 0.0) );
		float4	c3	= gl.texture.Sample( un_Texture, uv + float2(0.0,  off.y) );
		float4	c4	= gl.texture.Sample( un_Texture, uv + float2(0.0, -off.y) );

		out_Color = (c0 + c1 + c2 + c3 + c4) / 6.0;
	}
...
//-------------------------------------------------------------------------------------------------
	...
	{
		float2		uv			= gl.FragCoord.xy * un_PerPass.invResolution;
	//	float2		off			= 1.0 / float2(gl.texture.GetSize( un_HalfSize, 0 ));
	//	float2		off			= 1.0 / un_PerPass.resolution.xy * 1.0/0.5;
		...
		// filter
		{
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2(-off.x, -off.y) ).rgb * (1.0/6.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2( off.x, -off.y) ).rgb * (1.0/6.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2(-off.x,  off.y) ).rgb * (1.0/6.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2( off.x,  off.y) ).rgb * (1.0/6.0);

			half_sized += gl.texture.Sample( un_HalfSize, uv + float2( 0.0,   -off.y)*2.0 ).rgb * (1.0/12.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2(-off.x,   0.0 )*2.0 ).rgb * (1.0/12.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2( off.x,   0.0 )*2.0 ).rgb * (1.0/12.0);
			half_sized += gl.texture.Sample( un_HalfSize, uv + float2( 0.0,    off.y)*2.0 ).rgb * (1.0/12.0);
		}
