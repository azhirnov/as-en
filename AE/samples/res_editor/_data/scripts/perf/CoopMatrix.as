...
//-----------------------------------------------------------------------------
	...
	#define CoopMatC	gl::CoopMat< half, gl::Scope::Subgroup, M, N, gl::MatrixUse::C >
	...
	void  Main ()
	{
		...
		[[unroll]] for (uint i = 0; i < COUNT; ++i)
		{
			...
			half	a = s_MatR[ (i + j + 1) % s_MatR.length() ];
			half	b = s_MatR[ (i + j + 2) % s_MatR.length() ];
			half	c = s_MatR[ (i + j + 3) % s_MatR.length() ];

			s_MatA[ (i + j) % s_MatA.length() ] += a;
			s_MatB[ (i + j) % s_MatB.length() ] += b;
