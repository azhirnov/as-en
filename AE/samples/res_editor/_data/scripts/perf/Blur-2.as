...
//-----------------------------------------------------------------------------
	...
	//	const EPixelFormat	hdr_fmt	= EPixelFormat::R11G11B10F;	// HDR on mobile
		...
		// render loop
		{
			...
			// Downscale
			{
				RC<Postprocess>		pass = Postprocess( "", "DOWNSAMPLE_HOR_BLUR" );
				...
				RC<Postprocess>		pass = Postprocess( "", "DOWNSAMPLE_VERT_BLUR" );
...
//-------------------------------------------------------------------------------------------------
	...
	//	float2	inv_res	= 1.0 / un_PerPass.resolution.xy;
	//	float2	inv_res	= 1.0 / float2(gl.texture.GetSize( un_Texture, 0 ));
		float2	inv_res	= un_PerPass.invResolution;

		out_Color  = Blur9(  un_Texture, uv, inv_res, float2(1.0, 0.0) );
	}
...
//-------------------------------------------------------------------------------------------------
	...
	{
		float2	uv		= gl.FragCoord.xy * un_PerPass.invResolution;
	//	float2	inv_res	= 1.0 / (un_PerPass.resolution.xy * 2.0);
	//	float2	inv_res	= 1.0 / float2(gl.texture.GetSize( un_Texture, 0 ));
		float2	inv_res	= un_PerPass.invResolution * 0.5;

		out_Color  = Blur9(  un_Texture, uv, inv_res, float2(0.0, 1.0) );
	}
...
//-------------------------------------------------------------------------------------------------
	...
	{
		float2	uv			= gl.FragCoord.xy * un_PerPass.invResolution;
		float3	half_sized	= gl.texture.Sample( un_HalfSize, uv ).rgb;
