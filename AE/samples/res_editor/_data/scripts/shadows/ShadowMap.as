...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		// render loop
		{
			RC<SceneGraphicsPass>	draw = scene.AddGraphicsPass( "shadow map pass" );
			draw.AddPipeline( "*-pass1.ppln" );
			draw.Output( sm, DepthStencil( 1.f, 0 ));
		}
		{
			RC<SceneGraphicsPass>	draw = scene.AddGraphicsPass( "main pass" );
			draw.AddPipeline( "*-pass2.ppln" );
			draw.Output( "out_Color",	rt,		RGBA32f( 0.3, 0.5, 1.0, 1.0 ));
			draw.Output(				ds,		DepthStencil( 1.f, 0 ));

			string	sampler = Sampler_NearestClamp;  // linear sampler may be unsupported
			if ( GetFeatureSet().hasLinearSampledFormat( sm.PixelFormat() ))
				sampler = Sampler_LinearClamp;

			draw.ArgIn( "un_ShadowMap",	sm,		sampler );
