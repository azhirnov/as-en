...
*/
	template <typename T>
	Bytes  RDataSourceRange<T>::ReadBlock (Bytes offset, OUT void *buffer, Bytes size) __NE___
	{
		ASSERT( IsOpen() );

		size = Min( offset + size, _size ) - offset;

		return _dataSource->ReadBlock( _offset + offset, OUT buffer, size );
	}
//-----------------------------------------------------------------------------
...
*/
	template <typename T>
	Bytes  WDataSourceRange<T>::Reserve (Bytes) __NE___
	{
		// TODO
		return 0_b;
	}
...
*/
	template <typename T>
	Bytes  WDataSourceRange<T>::WriteBlock (Bytes offset, const void *buffer, Bytes size) __NE___
	{
		ASSERT( IsOpen() );
		return _dataSource->WriteBlock( _offset + offset, buffer, size );
	}
