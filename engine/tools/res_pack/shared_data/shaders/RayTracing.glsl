...
ND_ Ray     Ray_FromScreen (const float3 origin, const quat rotation, const float fovX, const float nearPlane, const int2 screenSize, const int2 screenCoord);
ND_ Ray     Ray_From (const float3 leftBottom, const float3 rightBottom, const float3 leftTop, const float3 rightTop, const float3 origin, const float nearPlane, const float2 unormCoord);
...
Ray  Ray_Create (const float3 origin, const float3 direction, const float tmin)
{
    ...
    result.pos      = FusedMulAdd( result.dir, float3(result.t), result.origin );
    return result;
}
