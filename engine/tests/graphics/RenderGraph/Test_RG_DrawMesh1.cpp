...
namespace
{
	...
	public:
		...
		DM1_DrawTask (DM1_TestData& t, RC<CommandBatch> batch, StringView dbgName) :
	...
	public:
		...
		DM1_CopyTask (DM1_TestData& t, RC<CommandBatch> batch, StringView dbgName) :
			RenderTask{ batch, dbgName },
			t{ t }
		{}
	...
	static bool  DrawTest (RenderTechPipelinesPtr renderTech, ImageComparator *imageCmp)
	{
		...
		AsyncTask	task1	= t.batch->Add< DM1_DrawTask<CtxType> >( MakeTuple(ArgRef(t)), MakeTuple(begin), "Draw task" );
		CHECK_ERR( task1 );
		
		AsyncTask	task2	= t.batch->Add< DM1_CopyTask<CopyCtx> >( MakeTuple(ArgRef(t)), MakeTuple(task1), "Readback task" );
		CHECK_ERR( task2 );
