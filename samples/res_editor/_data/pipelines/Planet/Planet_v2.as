...
//-----------------------------------------------------------------------------
    ...
    {
        {
            RC<ShaderStructType>    st = ShaderStructType( "planet2.io.vs-tcs" );
            st.Set( "float4     position;" +
                    "float3     texcoord;" );
        }{
            RC<ShaderStructType>    st = ShaderStructType( "planet2.io.tcs-tes" );
            st.Set( "float4     position;" +
                    "float3     texcoord;" );
        }{
            RC<ShaderStructType>    st = ShaderStructType( "planet2.io.tes-fs" );
            st.Set( "float3     texcoord;" );
        }

        {
            RC<DescriptorSetLayout> ds = DescriptorSetLayout( "planet2.mtr.ds" );
            ...
            ds.UniformBuffer( EShaderStages::AllGraphics,   "un_PerObject",     ArraySize(1),           "SphericalCubeMaterialUB" );
        }{
            RC<PipelineLayout>      pl = PipelineLayout( "planet2.pl" );
            pl.DSLayout( "pass",     0, "pass.ds" );
            pl.DSLayout( "material", 1, "planet2.mtr.ds" );
        }

        {
            RC<GraphicsPipeline>    ppln = GraphicsPipeline( "planet2.draw" );
            ppln.SetLayout( "planet2.pl" );
            ppln.SetVertexInput( "VB{SphericalCubeVertex}" );
            ppln.SetFragmentOutputFromRenderTech( "rtech", "main" );
            ppln.SetShaderIO( EShader::Vertex,          EShader::TessControl,    "planet2.io.vs-tcs" );
            ppln.SetShaderIO( EShader::TessControl,     EShader::TessEvaluation, "planet2.io.tcs-tes" );
            ppln.SetShaderIO( EShader::TessEvaluation,  EShader::Fragment,       "planet2.io.tes-fs" );
            ...
            // specialization
            {
                RC<GraphicsPipelineSpec>    spec = ppln.AddSpecialization( "planet2.draw" );
...
//-----------------------------------------------------------------------------
    ...
    void Main ()
    {
        ...
        gl.Position  = un_PerPass.camera.viewProj * (un_PerObject.transform * pos);
    }
