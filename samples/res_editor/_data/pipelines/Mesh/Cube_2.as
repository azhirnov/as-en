...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		// pipeline layout
		{
			...
			ds.UniformBuffer( EShaderStages::MeshTask | EShaderStages::Mesh, "mtrUB", ArraySize(1), "UnifiedGeometryMaterialUB" );
...
//-----------------------------------------------------------------------------
	...
	ND_ bool  IsVisible (float3 center, float radius) {
		return Frustum_TestSphere( passUB.camera.frustum, center, radius );
	}
	...
	void Main ()
	{
		...
		const float3	pos			= float3(task_pos.x, Sin( passUB.time + time_offset ), task_pos.y );
		const float3	vpos		= (mtrUB.transform * float4(pos, 1.0)).xyz;
...
//-----------------------------------------------------------------------------
	...
	//	const float		LOD		= In.payload[gid].lod;

		gl.MeshVertices[I].gl_Position	= passUB.camera.viewProj * mtrUB.transform * pos;
