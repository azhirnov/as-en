    ...
    {
        {
            ...
            ds.StorageBuffer( EShaderStages::Vertex,    "un_Geometry",          ArraySize(1), "GeometrySBlock", EAccessType::Coherent, EResourceState::ShaderStorage_Read );    // external
...
//-----------------------------------------------------------------------------
    ...
    void Main ()
    {
        ...
        const float3x3  norm_mat = float3x3(un_PerObject.transform);

        gl.Position     = pos;
        Out.texcoord    = un_Geometry.texcoords[idx];
        Out.normal      = norm_mat * un_Geometry.normals[idx];
        Out.tangent     = norm_mat * un_Geometry.tangents[idx];
        Out.bitangent   = norm_mat * un_Geometry.bitangents[idx];
    }
