...
namespace AE::Samples::Demo
{
    ...
    // Upload Task
    //
    class RayTracingSample::UploadTask final : public RenderTask
    {
    public:
        RC<RayTracingSample>    t;

        UploadTask (RayTracingSample* p, CommandBatchPtr batch, DebugLabel) :
            RenderTask{ batch, {"RayTracing::Upload"} },
            t{ p }
        {}
...
=================================================
*/
    void  RayTracingSample::UploadTask::Run ()
    {
...

        {
            ...
            // update descriptors
            {
                auto [ds, idx] = res_mngr.CreateDescriptorSet( t->ppln, DescriptorSetName{"rtrace1.ds0"} );
...
//-----------------------------------------------------------------------------
    ...
    // Ray Tracing Task
    //
    class RayTracingSample::RayTracingTask final : public RenderTask
    {
    public:
        RC<RayTracingSample>    t;
        IOutputSurface &        surface;

        RayTracingTask (RayTracingSample* p, IOutputSurface &surf, CommandBatchPtr batch, DebugLabel) :
            RenderTask{ batch, {"RayTracing::Trace"} },
            t{ p }, surface{ surf }
        {}
...
=================================================
*/
    void  RayTracingSample::RayTracingTask::Run ()
    {
...
=================================================
*/
    bool  RayTracingSample::Init (PipelinePackID pack)
    {
        auto&   res_mngr = RenderTaskScheduler().GetResourceManager();
        ...
        rtPipelines = res_mngr.LoadRenderTech( pack, RenderTechName{"RayTracing.RTech"}, Default );
        CHECK_ERR( rtPipelines );

        ppln = rtPipelines->GetRayTracingPipeline( PipelineName{"rtrace1.def"} );
        CHECK_ERR( ppln );

        sbt = rtPipelines->GetRTShaderBinding( RTShaderBindingName{"rtrace1.sbt0"} );
        CHECK_ERR( sbt );
...
=================================================
*/
    AsyncTask  RayTracingSample::Draw (RenderGraph &rg, ArrayView<AsyncTask> inDeps)
    {
        auto    batch = rg.Render( "RayTracing pass" );
        CHECK_ERR( batch );
        ...
        return batch->Run< RayTracingTask >( Tuple{ this, rg.GetSurfaceArg() }, Tuple{surf_acquire}, True{"Last"}, Default );
    }
