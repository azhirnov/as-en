...
namespace AE::Samples::Demo
{
	...
	public:
		...
		UploadTextureTask (Canvas2DSample* t, CommandBatchPtr batch, DebugLabel) :
...
//-----------------------------------------------------------------------------
	...
	public:
		...
		UploadAtlasTask (Canvas2DSample* t, CommandBatchPtr batch, DebugLabel) :
...
//-----------------------------------------------------------------------------
	...
	public:
		...
		ProcessInputTask (Canvas2DSample* t, ActionQueueReader reader) :
...
*/
	void  Canvas2DSample::ProcessInputTask::Run ()
	{
		...
		{
			if ( hdr.name == InputActionName{"Cursor"} )
				t.cursorPos = reader.Data<packed_float2>( hdr.offset );

			if ( hdr.name == InputActionName{"Push"} )
				t.enter = true;
		}
	}
//-----------------------------------------------------------------------------
	...
	public:
		...
		DrawTask (Canvas2DSample* t, IOutputSurface &surf, CommandBatchPtr batch, DebugLabel) :
